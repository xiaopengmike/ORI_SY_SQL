...
class Retrieve extends Action
{
    private function queryData()
    {
        ...
        if(isset($_SERVER["HTTP_X_REQUESTED_WITH"])){
            ...
            include("lang.php");
            $msg=$_REQUEST['id']?$LG_CUSTOMER_DATA['修改'][$_COOKIE['LANG']]:$LG_CUSTOMER_DATA['添加'][$_COOKIE['LANG']];
            if($result['code']==200){
                Message("", $msg." ".$LG_CUSTOMER_DATA['成功'][$_COOKIE['LANG']]);
                Button_Close();
                echo '<script language=JavaScript> self.opener.location.reload(); </script>';
                exit;
            }else{
                Message("错误", $msg." ".$LG_CUSTOMER_DATA['失败'][$_COOKIE['LANG']]);
                Button_Back();
                exit;
            }
        }
