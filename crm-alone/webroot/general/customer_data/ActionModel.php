...
class ActionModel
{
    ...
    public function queryById($param)
    {
        ...
        $param['customer_id'] = $param[customer_id]?$param[customer_id]:$main['id'];  // ȡ�ͻ�ID�ţ���ֱ��ȡform_id����
        ...
        if($is_nergy=="INHENERGY"&&empty($param['form_type'])){
            if($param['addContactCount']==0){
                $result = array(
                    'code' => '0',
                    'status' => 'failed',
                    'msg' => $LG_CUSTOMER_DATA['��ϵ�˲���Ϊ�գ�'][$_COOKIE['LANG']],
                    'data' => '��ϵ�˲���Ϊ�գ�'
                );
                return $result;
            }
            ...
            if($contacts_phone_flag){
                $result = array(
                    'code' => '0',
                    'status' => 'failed',
                    'msg' => $LG_CUSTOMER_DATA['�绰�������䲻��Ϊ�գ�'][$_COOKIE['LANG']],
                    'data' => '�绰�������䲻��Ϊ�գ�'
                );
                return $result;
            }
        ...
        if (array_key_exists('action', $result)) {
            if(!empty($result['param']["customer_name"])&&!$this->verifyNotExist($result['param'],'customer_name')){
                $result = array(
                    'code' => '0',
                    'status' => 'failed',
                    'msg' => $LG_CUSTOMER_DATA['�ͻ��Ѵ���'][$_COOKIE['LANG']],
                    'data' => '�ͻ������Ѿ����ڣ���������д'
                );
                return $result;
            }
            if(!empty($result['param']["short_name"])&&!$this->verifyNotExist($result['param'],'short_name')){
                $result = array(
                    'code' => '0',
                    'status' => 'failed',
                    'msg' => $LG_CUSTOMER_DATA['����Ѵ���?][$_COOKIE['LANG']],
                    'data' => '����Ѿ����ڣ����������?
                );
                return $result;
            }
