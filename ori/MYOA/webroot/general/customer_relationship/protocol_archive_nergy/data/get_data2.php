...
$selectParam = array(
    'is_used' => '1', // 可用参数
    'type' => 'yes_or_no,contract_protocol_type,continent,manage_dept_nergy,currency',
);
...
$sql = " select m.*, pp.paras_desc contract_category_desc, t.filename foreignAttachName, tt.filename chineseAttachName  from inhe_sales_contract_nergy m "
    . "left join inhe_oa_paras pp on pp.type = 'contract_category' and pp.paras_value = m.contract_category "
    . "left join inhe_attachment t on t.type = 'foreign_attach_nergy' and t.relatedId = m.id "
    . "left join inhe_attachment tt on tt.type = 'chinese_attach_nergy' and tt.relatedId = m.id "
    . $where . $orderBy . $pageLimit;
$countSql = " select count(*) total_count from inhe_sales_contract_nergy m "
. "left join inhe_oa_paras pp on pp.type = 'contract_category' and pp.paras_value = m.contract_category "
    . "left join inhe_attachment t on t.type = 'foreign_attach_nergy' and t.relatedId = m.id "
    . "left join inhe_attachment tt on tt.type = 'chinese_attach_nergy' and tt.relatedId = m.id "
. $where . $orderBy;
...
while ($item = mysql_fetch_assoc($res)) {
    $item['foreign_attach_nergy'] = $item['foreign_attach_nergy'] == '' ? $item['foreignAttachName'] : $item['foreign_attach_nergy'];
    $item['chinese_attach_nergy'] = $item['chinese_attach_nergy'] == '' ? $item['chineseAttachName'] : $item['chinese_attach_nergy'];
