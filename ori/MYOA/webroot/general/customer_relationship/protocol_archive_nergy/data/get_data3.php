...
$systemId = 'authorization_letter_nergy';
...
$sql = " select m.*, t.filename applyAttachName, tt.filename bankAttachName, ttt.filename transferAttachName from inhe_authorization_letter_nergy m "
    // . "left join inhe_oa_paras pp on pp.type = 'bid_type' and pp.paras_value = m.contract_category "
    . "left join inhe_attachment t on t.type = 'apply_attach_nergy' and t.relatedId = m.id "
    . "left join inhe_attachment tt on tt.type = 'bank_attach_nergy' and tt.relatedId = m.id "
    . "left join inhe_attachment ttt on ttt.type = 'transfer_attach_nergy' and ttt.relatedId = m.id "
    . $where . $orderBy . $pageLimit;
$countSql = " select count(*) total_count from inhe_authorization_letter_nergy m ". $where . $orderBy;
// var_export($sql);exit;
...
while ($item = mysql_fetch_assoc($res)) {
    $item['apply_attach_nergy'] = $item['apply_attach_nergy'] == '' ? $item['applyAttachName'] : $item['apply_attach_nergy'];
    $item['bank_attach_nergy'] = $item['bank_attach_nergy'] == '' ? $item['bankAttachName'] : $item['bank_attach_nergy'];
    $item['transfer_attach_nergy'] = $item['transfer_attach_nergy'] == '' ? $item['transferAttachName'] : $item['transfer_attach_nergy'];
