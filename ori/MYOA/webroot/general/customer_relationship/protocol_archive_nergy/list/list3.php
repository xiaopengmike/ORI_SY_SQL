...
<body style="background-color: white;">
    ...
    <script>
                ...
                cols: [
                    [
                        ...
                        // {field: 'serial_number',title: '序号',width: 60,align: 'center',fixed: true},
                        {
                            ...
                            field: 'apply_attach_nergy',
                            title: '最终保函申请资料',
                            templet: applyAttachLink,
                            align: 'center'
                        }, {
                            field: 'bank_attach_nergy',
                            title: '国内银行电文',
                            templet: bankAttachLink,
                            align: 'center'
                        }, {
                            field: 'transfer_attach_nergy',
                            title: '转开行最终保函',
                ...
                switch (obj.event) {
                    case 'delete':
                        ...
                        layer.confirm('确定要删除当前记录吗？', {
                            ...
                            var url = '../action.php?action=Delete&id=' + deleteData.id + '&tableName=inhe_authorization_letter_nergy';
        ...
        function applyAttachLink(d) {
            var attachName = d.apply_attach_nergy;
            ...
            if (attachName.length > 0) {
                var attachUrl = '<a class="layui-blue" href="javascript:void(0);" lay-event="file" onclick="openFullModel(\'/attachment/reportshop/attachment/' +
                    d.apply_attach_nergy + '\');return false;">附件下载</a>';
                return attachUrl;
            }
        }

        function bankAttachLink(d) {
            var attachName = d.bank_attach_nergy;
            if ('' == attachName || null == attachName || undefined == attachName) {
                return '';
            }
            if (attachName.length > 0) {
                var attachUrl = '<a class="layui-blue" href="javascript:void(0);" lay-event="file" onclick="openFullModel(\'/attachment/reportshop/attachment/' +
                    d.bank_attach_nergy + '\');return false;">附件下载</a>';
                return attachUrl;
            }
        }

        function transferAttachLink(d) {
            var attachName = d.transfer_attach_nergy;
            if ('' == attachName || null == attachName || undefined == attachName) {
                return '';
            }
            if (attachName.length > 0) {
                var attachUrl = '<a class="layui-blue" href="javascript:void(0);" lay-event="file" onclick="openFullModel(\'/attachment/reportshop/attachment/' +
                    d.transfer_attach_nergy + '\');return false;">附件下载</a>';
                return attachUrl;
            }
        }
    </script>
