...
//在上一个审核页面已经判断下一步是否董事长步骤，是则将步骤改为当前操作人步骤+1了
// if(strpos($param2['form_id'],strtolower($param2['user_bu']).'_')===0&&$flowArr[$param2[step]]['approver']=='meterw'&&$param2['user_bu']!="INHE"){
...
$param['tableName'] = 'inhe_contract_review';
...
if ($param['actionType'] == 'Back' || $ifAgree == FormModel::DISAGREE) {
    ...
    if(strpos($param['form_id'],strtolower($param['user_bu']).'_')===0){
        $approverArr = $processModel->skipApprover($flow_manage_param);
        $param[step]=$approverArr[step]; 
        //var_dump($approverArr);exit;
    }
}
//获取当前审核角色
...
if (empty($approverArr) || count($approverArr) == 0) {
    ...
    $sql = "update inhe_contract_review set status = 'Y' where form_id = '" . $param['form_id'] . "' ";
...
$param['tableName'] = 'inhe_contract_detail';
$formData = $formModel->query($param);
$dataDetail = $formData[0];
...
and exists(select * from inhe_product_detail where form_id='$data[form_id]' and find_in_set(k.user_bu,other_company_product)) ";
    ...
    <form id="mainForm" class="layui-form">
        ...
        <div class="layui-form-item">
            ...
            <?php foreach ($approverArr as $app) {
                ...
                // if($actionType=='退回' && ($data['user_bu']=='INHENERGY'||$data['user_bu']=='HKNERGY') && $param['step']>=17 && $app['step']<17){
                //     continue;
                // }
                if ($approveRole == '商务总监') {
                ...
                if ($approveRole == '法务'&&$actionType == '提交') {
                    //成都走电力流程特殊处理
                    if($data['user_bu']=="INHE"&&$data['create_user_bu']=="CDINHE"){
                        if ($app['role'] != '成都董事长') {
                            continue;
                ...
                //成都走电力流程特殊处理
                if($data['user_bu']=="INHE"&&$data['create_user_bu']=="CDINHE"){
                    if ($approveRole == '成都董事长'&&$actionType == '提交') {
                        if ($param['if_eligible'] == '02' && $param['if_chief_contract'] == '01' && $app['role'] == '备案') {
                            continue;
                        }
                        if ($param['if_eligible'] == '01' && $param['if_chief_contract'] != '01' && $app['role'] == '董事长') {
                ...
                //人民币>=350W 美元>=50W
                if (in_array($data['user_bu'],array("INHEIAC","INHEIMC"))&&$approveRole == '财务经理'&&$actionType == '提交') {
                    if (((float)$dataDetail[total_amount]>=500000&&$data[currency]=="02")||((float)$dataDetail[total_amount]>=3500000&&$data[currency]=="04")) {
                        if ($app['role'] != '财务总监'){
                            continue;
                        }
                    }else{
                ...
                //人民币>=350W 美元>=50W
                if (in_array($data['user_bu'],array("WITLINK","JXINHE","INHEJX"))&&$approveRole == '总经理'&&$actionType == '提交') {
                    if ((((float)$dataDetail[total_amount]>=500000&&$data[currency]=="02")||((float)$dataDetail[total_amount]>=3500000&&$data[currency]=="04"))) {
                        if ($app['role'] != '集团财务总监'){
                            continue;
                        }
                    ...
                    // if(strpos($param2['form_id'],strtolower($param2['user_bu']).'_')===0){
                    //     if ($app['role'] != '备案'){
                    //         continue;
                    //     }
                    // }else{
                ...
                // if(strpos($param2['form_id'],strtolower($param2['user_bu']).'_')===0&&$currApproverData2[0]['auto_approver']=="1"&&$currApproverData2[0]['operate']!='record'){
                //     $role_names = array_column($approverArr, 'role');
                ...
                // if ($approveRole == '风险控制与项目融资部'&&($data['user_bu']!='INHENERGY'||$data['user_bu']!='HKNERGY')) {
                //     if ($param['if_eligible'] == '02' && $param['if_chief_contract'] == '01' && $app['role'] == '备案') {
                //         continue;
                //     }
                //     if ($param['if_eligible'] == '01' && $param['if_chief_contract'] != '01' && $app['role'] == '董事长') {
                ...
                //     }
                // }
                // if (in_array($approveRole,array('市场总监','总部商务','电表技术评审','配电产品技术评审','系统软件技术评审')) &&($data['user_bu']=='INHENERGY'||$data['user_bu']=='HKNERGY') && $actionType=='提交' ) {
                ...
                //     }
                // }
                // if ($approveRole == '出纳'&&($data['user_bu']=='INHENERGY'||$data['user_bu']=='HKNERGY') && $actionType=='提交') {
