...
echo '<div id="treediv" style="display: none;position:absolute;overflow:hidden;min-width: 250px;min-height:200px;  padding: 5px;background: #fff;color: #fff;border: 1px solid #718A9B">
	...
	<script type="text/javascript">
		...
		var toshow = "treediv";
		var target = "dept_select";

		$("input[data-type=\'dept_select\']").each(function() {
			this.onclick = function() {
				$("#" + toshow)
				.css("position", "absolute")
				.css("left", $(this).offset().left + xOffset + "px")
				.css("top", $(this).offset().top + yOffset + "px").show();
				$("#dept_number").val(this.title);
			};
		});
		...
		function setvalue(id, name) {
			...
			$("#menu_parent_name"+dept_number).val(name);
			$("#menu_parent"+dept_number).val(id);
			$("#treediv").hide();
			selectUserByDeptId(id,dept_number);
		}
    </script>';
