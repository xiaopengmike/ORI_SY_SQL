    ...
    public function exportExcel($result, $titleArr, $workSheetName)
    {
        ...
        if (Count($result) > 0) {
            ...
            foreach ($titleArr as $k => $value) {
                ...
                $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow($index, 2)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
                $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow($index, 2)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
            ...
            for ($i = 0; $i < count($result); $i++) {
                ...
                foreach ($titleArr as $k => $value) {
                    ...
                    // 设置垂直（VERTICAL_CENTER）水平（HORIZONTAL_CENTER）居中
                    $objPHPExcel->getActiveSheet()->getStyleByColumnAndRow($j, $row + $i)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
