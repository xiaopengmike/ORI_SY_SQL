...
class ActionModel
{
    public $systemId = "sample_request";
    ...
    public function submit()
    {
        ...
        $param['param_templ']=td_json_encode($param['param_templ']);
        $form = new FormModel();
...
public function end()
{
    ...
    // $tempSql = " select * from inhe_process_travel_out where id=$param[id] ";
    // $res=exequery(TD::conn(), $tempSql);
    // if($item = mysql_fetch_assoc($res)){
    //     if($res[destination_type_status]=="C"){
    //         $result = $response->failed('流程已取消，不可再操作');
    //         return $result;
    //     }
    //     if($res[destination_type_status]=="E"||$res[destination_type_status]=="Y"){
    //         $result = $response->failed('流程已结束，不可再操作');
    //         return $result;
    //     }
    // }
    $result = $response->failed('流程已结束，不可再操作');
   ...
   /**
     * 更新料号,流程结束
     */
    public function updatePartNumeber()
    {
        $param = $this->getParamToArray();
       
        if(isset($param[part_number])&&!empty($param[part_number])){
            $sql = " update inhe_sample_request set part_number='$param[part_number]',pm_status='$param[pm_status]',status='E'  where form_id = '$param[form_id]' ";
            exequery(TD::conn(), $sql);  
        }
        $time = date('Y-m-d H:i:s');
        $sql = " update inhe_flow_opinion set approve_user='$_SESSION[LOGIN_USER_ID]',approve_time='$time', status='H' where form_id = '$param[form_id]' and step='900'";
        exequery(TD::conn(), $sql);
        $time = date('Y-m-d H:i:s');
        $sql = " update inhe_flow_opinion set approve_time='$time', status='H' where form_id = '$param[form_id]' and status='U'";
        exequery(TD::conn(), $sql);
        $response = new ResponseCode();
        $result = $response->failed('料号申请成功');
        $result = $response->success($result);
        return $result;
    }
...
/**
     * 更新料号,流程结束
     */
    public function updatePurchasePartNumeber()
    {
        $param = $this->getParamToArray();
       
        if(isset($param[part_number])&&!empty($param[part_number])){
            $sql = " update inhe_sample_request set part_number='$param[part_number]',pm_status='$param[pm_status]'  where form_id = '$param[form_id]' ";
            exequery(TD::conn(), $sql);  
        }
        $time = date('Y-m-d H:i:s');
        $sql = " update inhe_flow_opinion set approve_user='$_SESSION[LOGIN_USER_ID]',approve_time='$time', status='H' where form_id = '$param[form_id]' and step='800'";
        exequery(TD::conn(), $sql);
        $time = date('Y-m-d H:i:s');
        $sql = " update inhe_flow_opinion set approve_time='$time', status='H' where form_id = '$param[form_id]' and status='U'";
        exequery(TD::conn(), $sql);
    ...
    public function addSampleComfirm($param)
    {
        
        if (!isset($param['addLicenseProductCount'])) {
            return true;
        }
        exequery(TD::conn(), "delete from inhe_sample_comfirm where sup_form_id='".$param['form_id']."'");
        exequery(TD::conn(), "delete from inhe_flow_opinion where form_id like 'syqr".$param['form_id']."%'");
       
        if ($param['addLicenseProductCount'] <= 0) {
            return true;
        }
        $main = $this->queryMain(array("id"=>$param['form_id']));
        if ($main['status'] !='P') {
            return true;
        }
        ...
        for ($i = 1; $i <= $param['addLicenseProductCount']; $i++) {
            if(!empty($param['supplier_name_'.$i])){
                $supplier=$param['supplier_name_'.$i];
                $arrival_time=$param['supplier_date_'.$i];
                $supplier_price=$param['supplier_price_'.$i];
                $procurement_cycle=$param['procurement_cycle_'.$i];
                $form_id=str_replace("syxq", "syqr", $param['form_id']).$j;
                $title=str_replace($param['form_id'], $form_id, $main['title']).$j;
                $valueSql.="('" . $title . "', '" . $form_id . "', '" . $param['other_explain'] .  "', '" . $supplier_price .  "', '" . $procurement_cycle . 
                "', '" . $supplier . "','" . $arrival_time ."','" . $main['project_code'] ."','" . $main['material_name'] .
                "', '" . $_SESSION[LOGIN_DEPT_ID] . "','" . $_SESSION[LOGIN_USER_ID] . "', '" . $create_time .
                "', 'P', 'sample_comfirm', '" .
                $param['user_bu'].  "', '" .$param['form_id'] . "'),";
                $valueSql2.="";
                ...
                values ('sample_comfirm','$form_id','$param[title]','$_SESSION[LOGIN_USER_ID]','$create_time','1','采购工程师','$_SESSION[LOGIN_USER_ID]','$create_time','H','$_SESSION[LOGIN_USER_ID]','$create_time','$param[user_bu]')";
                exequery(TD::conn(), $sql);
                
                $sql = " insert into inhe_flow_opinion (type, form_id,title, user_id,write_time, step, work_flow, approve_user,approve_time, status,prev_writer,prev_time, user_bu) 
                values ('sample_comfirm','$form_id','$param[title]','$_SESSION[LOGIN_USER_ID]','$create_time','2','上传规格书','$_SESSION[LOGIN_USER_ID]','','U','$_SESSION[LOGIN_USER_ID]','$create_time','$param[user_bu]')";
                exequery(TD::conn(), $sql);
                $j++;
            }
        }
        ...
        if($valueSql){
            $sql="insert into inhe_sample_comfirm (title, 
            form_id, 
            other_explain, 
            supplier_price, 
            procurement_cycle, 
            ...
            exequery(TD::conn(), $sql);
            $j=$j-1;
            $sql = " update inhe_sample_request set supplier_number='$j',purchasing_engineer='$_SESSION[LOGIN_USER_ID]'  where form_id = '" . $param['form_id'] . "' ";
            exequery(TD::conn(), $sql);
            $REMIND_URL = "/customer_relationship/sample_request/sample_comfirm/list.php?request_id=" . $param['form_id'];
            $SMS_CONTENT = $main['title']. _("，索样确认上传规格书！");
            send_sms($REMIND_TIME, $_SESSION["LOGIN_USER_ID"], $_SESSION["LOGIN_USER_ID"], 0, $SMS_CONTENT, $REMIND_URL);
        }
    ...
    public function updateApproveOpinion()
    {
        ...
        $this->addSampleComfirm($param);
        if(isset($param[supplier_number])&&!empty($param[supplier_number])){
            $sql = " update inhe_sample_request set supplier_number='$param[supplier_number]',purchasing_engineer='$_SESSION[LOGIN_USER_ID]'  where form_id = '" . $param['form_id'] . "' ";
            exequery(TD::conn(), $sql);
        }
        if(isset($param[part_number])&&!empty($param[part_number])){
            $sql = " update inhe_sample_request set part_number='$param[part_number]',pm_status='$param[pm_status]'  where form_id = '$param[form_id]' ";
    ...
    {
        $mainSql = "insert into inhe_sample_request (title, 
        form_id, 
        other_explain, 
        material_name,
        material_number,
        m_specification_model,
        m_parameter_description,
        m_drawing_number,
        ...
        user_bu
            ...
            "', '" . $param['material_name'] . "','" . $param['material_number'] . "', '" . $param['m_specification_model'] .
            "', '" . $param['m_parameter_description'] . "','" . $param['m_drawing_number'] . "', '" . $param['recommended_supplier'] .
            "', '" . $param['expected_arrival_time'] . "','" . $param['project_code'] . "','" . $param['param_templ'] ."','" . $param['param_templ_v'] .
            "', '" . $param['material_flow'] . "','" . $param['material_type'] ."','" . $param['mold_opening'] . 
    ...
    public function updateMainData($param)
    {
        ...
        $paramInsert = array(
            ...
            'material_number' => $param['material_number'],
            'm_specification_model' => $param['m_specification_model'],
            'm_parameter_description' => $param['m_parameter_description'],
            'm_drawing_number' => $param['m_drawing_number'],
            'recommended_supplier' => $param['recommended_supplier'],
            'expected_arrival_time' => $param['expected_arrival_time'],
            'project_code' => $param['project_code'],
            'param_templ_v' => $param['param_templ_v'],
            'param_templ' => $param['param_templ'],
            'material_flow' => $param['material_flow'],
            'material_type' => $param['material_type'],
            'mold_opening' => $param['mold_opening'],        
    ...
    public function delete()
    {
        ...
        $sql = " select k.* inhe_sample_comfirm  where sup_form_id ='$param[id]'";
        $res = exequery(TD::conn(), $sql);

        if ($item = mysql_fetch_assoc($res)) {
            $sample_comfirm = $item;
        }
        $sql = " delete from inhe_sample_request " . $where;
        $re = exequery(TD::conn(), $sql);
        $sql = " delete from inhe_flow_opinion  where  form_id = '" . $param['id'] . "' ";
        $res = exequery(TD::conn(), $sql);

        $sql = " delete from inhe_sample_comfirm  where form_id = '" . $sample_comfirm['form_id'] . "'";
        $res = exequery(TD::conn(), $sql);
        $sql = " delete from inhe_flow_opinion  where  form_id = '" . $sample_comfirm['form_id'] . "' ";
        $res = exequery(TD::conn(), $sql);
