...
class ActionModel
{
    public $systemId = "sample_comfirm";
    public static $NON_METER_TYPE = array('non_lieu');
    ...
    public function queryMain($param)
    {
        ...
        $sql = " select k.*,u.user_name createUserName,sr.user_id request_user_id,d.dept_name deptName from inhe_sample_comfirm k " .
            " left join inhe_sample_request sr on sr.form_id = k.sup_form_id " .
    ...
    public function queryOrderDetail($param)
    {
        ...
        $sql = " select * from inhe_lieureceive_detail k " . $strWhere . " order by id ";
        $res = exequery(TD::conn(), $sql);
        while ($item = mysql_fetch_assoc($res)) {
            $result['non_lieu'][] = $item;
        }
    ...
    public function submit()
    {
        ...
        if (array_key_exists('action', $result)) {
            ...
            $supplier_number=$this->verifyExist($result['param'],'supplier_number');
                if($supplier_number!=false){
                    $result = array(
                        'code' => '0',
                        'status' => 'failed',
                        'msg' => '填写记录数量不能超出供应商数量：'.$supplier_number,
                        'data' => '填写记录数量不能超出供应商数量：'.$supplier_number
                    );
                    return $result;
                }
            }
    ...
    public function updateApproveOpinion()
    {
        ...
        if(isset($param[mold_opening_time])&&!empty($param[mold_opening_time])){
            $sql = " update inhe_sample_comfirm set mold_opening_time='$param[mold_opening_time]',sample_return_time='$param[sample_return_time]'  where form_id = '" . $param['form_id'] . "' ";
            exequery(TD::conn(), $sql);
        }
        if(isset($param[sample_projected_time])&&!empty($param[sample_projected_time])){
            $sql = " update inhe_sample_comfirm set sample_projected_time='$param[sample_projected_time]'  where form_id = '" . $param['form_id'] . "' ";
            exequery(TD::conn(), $sql);
        }
        if(isset($param[sample_actual_time])&&!empty($param[sample_actual_time])){
            $sql = " update inhe_sample_comfirm set sample_actual_time='$param[sample_actual_time]' where form_id = '" . $param['form_id'] . "' ";
            exequery(TD::conn(), $sql);
        }

        if(isset($param[actual_return_time])&&!empty($param[actual_return_time])){
            $sql = " update inhe_sample_comfirm set actual_opening_time='$param[actual_opening_time]',actual_return_time='$param[actual_return_time]' where form_id = '" . $param['form_id'] . "' ";
            exequery(TD::conn(), $sql);
        }

        if(isset($param[end_type])&&!empty($param[end_type])){
            $sql = " update inhe_sample_comfirm set end_type='$param[end_type]',end_remark='$param[end_remark]' where form_id = '" . $param['form_id'] . "' ";
            exequery(TD::conn(), $sql);
            $sql = " update inhe_sample_request set part_number='$param[end_remark]' where form_id = '" . $param['request_id'] . "' ";
            exequery(TD::conn(), $sql);
        }

        if(isset($param[sample_no_reason])&&!empty($param[sample_no_reason])){
            $sql = " insert into inhe_sample_return_report(request_id,opinion_id,reason)values('$param[request_id]','$param[opinion_id]','$param[sample_no_reason]') ";
            exequery(TD::conn(), $sql);  
        }
    ...
    {
        $mainSql = "insert into inhe_sample_comfirm (title, 
        form_id, 
        other_explain, 
        supplier, 
        arrival_time,
        ...
        sup_form_id
            ...
            "', '" . $param['supplier'] . "','" . $param['arrival_time'] ."','" . $param['project_code'] ."','" . $param['material_name'] .
            "', '" . $param['organ_id'] . "','" . $param['user_id'] . "', '" . $param['write_time'] .
            "', '" . $param['status'] . "', '" . $param['type'] .  "', '" .
            $param['user_bu'].  "', '" .$param['sup_form_id'] . "')";
...
public function end()
{
    ...
    if(isset($param[form_id])&&!empty($param[form_id])){
        $prev_time=date("Y-m-d H:i:s");
        $where = " where form_id = '" . $param['form_id'] . "' and type ='sample_comfirm' and approve_user = '" . $_SESSION[LOGIN_USER_ID]. "' and status = 'U' ";
        $updateSql = " update inhe_flow_opinion set approve_time = '" . $prev_time . "', status = 'E'" . $where;
        exequery(TD::conn(), $updateSql);
        $processModel = new ProcessModel();
        $processModel->updateSampleRequest($param);
    }
    ...
    public function delete()
    {
        ...
        $sql = " select count(id)count from inhe_sample_comfirm where sup_form_id='$param[sup_form_id]' ";
        $res = exequery(TD::conn(), $sql);
        $sample_comfirm_count = 0;
        if ($item = mysql_fetch_assoc($res)) {
            $sample_comfirm_count=$item [count];
        }
        $sql = " delete from inhe_sample_comfirm " . $where;
        ...
        if($sample_comfirm_count<=1){
            $sql = " delete from inhe_flow_opinion  where type='sample_request' and step='800' and form_id = '" . $param['sup_form_id'] . "' ";
            $res = exequery(TD::conn(), $sql);
            $sql = "select max(id)opinion_id from inhe_flow_opinion where form_id = '$param[sup_form_id]'  and status='H' and type='sample_request' and work_flow='采购外发'";
            $res = exequery(TD::conn(), $sql);
            $opinion_id = "";
            if ($item = mysql_fetch_assoc($res)) {
                $opinion_id=$item [opinion_id];
            }
            $sql = " update inhe_flow_opinion set status='U' where form_id = '$param[sup_form_id]' and id =$opinion_id";
            $res = exequery(TD::conn(), $sql);
            $sql = " update inhe_sample_request set status='P' where form_id = '$param[sup_form_id]' ";
            $res = exequery(TD::conn(), $sql);
        }
     ...
     * $param 字段信息
     * 
     * $type  类型 excipients_detail_id 判断物料已审核
     */
    public function verifyExist($param,$type='supplier_number'){
        switch($type){
            case 'supplier_number':
                $supplier_number1=0;
                $supplier_number2=0;
                $tempSql = " select supplier_number from inhe_sample_request where form_id='$param[sup_form_id]'";
                $res=exequery(TD::conn(), $tempSql);
                if($item = mysql_fetch_assoc($res)){
                    $supplier_number1= (int)$item[supplier_number];
                }
                $tempSql = " select count(*)num from inhe_sample_comfirm where sup_form_id='$param[sup_form_id]'";
                $res=exequery(TD::conn(), $tempSql);
                if($item = mysql_fetch_assoc($res)){
                    $supplier_number2= (int)$item[num];
                }
                if($supplier_number1<=$supplier_number2){
                    return $supplier_number1;
                }
