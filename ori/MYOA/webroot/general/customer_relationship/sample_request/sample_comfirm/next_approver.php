...
if (empty($approverArr) || count($approverArr) == 0) {
    ...
    $sql = "update inhe_sample_comfirm set status = 'Y',closed = 'Y' where form_id = '" . $param['form_id'] . "' ";
...
$param['tableName'] = 'inhe_sample_comfirm';
$param['id'] = $param['form_id'];
$action = new ActionModel();
$data = $action->queryById($param);
$data = $data['main'];

$request_data = $formModel->query(array('tableName'=>'inhe_sample_request','form_id'=>$data[sup_form_id]));
$request_data = $request_data[0];



$sql = " select k.step,k.approve_user,u.user_name approverName from inhe_flow_opinion k 
left join user u on u.user_id = k.approve_user where k.type='sample_request' and k.form_id='$data[sup_form_id]' order by step,id";
$res = exequery(TD::conn(), $sql);
while ($item = mysql_fetch_assoc($res)) {
    $supFlowOpinionArr[$item['step']] = array("approverName"=>$item[approverName],"approve_user"=>$item[approve_user]);
}
$sql = " select k.step,k.approve_user,u.user_name approverName from inhe_flow_opinion k 
left join user u on u.user_id = k.approve_user where k.type='sample_comfirm' and k.form_id='$data[form_id]' order by step,id";
$res = exequery(TD::conn(), $sql);
while ($item = mysql_fetch_assoc($res)) {
    $flowOpinionArr[$item['step']] = array("approverName"=>$item[approverName],"approve_user"=>$item[approve_user]);
}
?>
...
<body>
    <form id="mainForm" class="layui-form">
        ...
        <input type="hidden" name="form_user_id" id="form_user_id" value="<?= $data['request_user_id'] ?>" />
        <input type="hidden" name="form_user_name" id="form_user_name" value="<?= rtrim(GetUserNameById($data['request_user_id']),",") ?>" />
        <input type="hidden" name="request_id" id="request_id" value="<?= $request_data['id'] ?>" />
        ...
        <div class="layui-form-item">
            ...
            <?php foreach ($approverArr as $app) {
                ...
                if(isset($approverArr[1])&&$actionType == '提交'){
                    ...
                    if($request_data[material_type]!='01'){
                        $param['项目经理']=in_array('项目经理',$role_names)?($request_data[mold_opening]=='02'?0:1):0;//开模是否需要跳过
                        $param['项目经理']=in_array('项目经理',$role_names)?($param['A']=='1'?0:$param['项目经理']):0;//变压器跳过
                        $param['开模']=in_array('开模',$role_names)?($request_data[mold_opening]=='02'?0:1):0;//变压器跳过
                    }
                    $param['正式样品测试']=in_array('正式样品测试',$role_names)?1:0;
    ...
    $(function() {
        layui.use(['form', 'layer'], function() {
            ...
            form.on('radio(step)', function(data) {
                ...
                $("#user_id").val('');
                $("#approver_list").val('');
                var create_user = $("#form_user_id").val();
                var create_user_name = $("#form_user_name").val();
                // 分3种情况：1、指定1人； 2、指定多人 3、选择一人
                if (approver.indexOf("step_") !== -1) {
                    
                    approver_arr=approver.split('_');
                    if(approver_arr[0]=="sup"){
                        supFlowOpinionArr=<?=td_json_encode($supFlowOpinionArr)?>;
                        returnUser(supFlowOpinionArr[approver_arr[2]]['approverName'], supFlowOpinionArr[approver_arr[2]]['approve_user'], 'approver_list')
                    }else{
                        supFlowOpinionArr=<?=td_json_encode($flowOpinionArr)?>;
                        returnUser(supFlowOpinionArr[approver_arr[1]]['approverName'], supFlowOpinionArr[approver_arr[1]]['approve_user'], 'approver_list')
                    }
