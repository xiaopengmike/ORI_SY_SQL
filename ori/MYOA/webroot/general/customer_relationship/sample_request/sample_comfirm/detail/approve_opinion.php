            ...
            <?php foreach ($opinion_attachment_arr[$fv['role']] as $ok => $ov) : ?>
                <?php if ($fv['step'] == $step && $operate == 'approve') : ?>
                    <tr>
                        ...
                        <td class="TableData" colspan="3"><input id="<?=$ok?>" name="<?=$ok?>" type="button" class="layui-btn layui-bg-gray btn-upload not_disabled" <?if($fv['role']=='上传规格书'):?>lay-verify="upload_attach"<?endif;?> style="display: inline!important;" value="上传"  /></td>
                    </tr>
                <?php else : ?>
                    <tr>
                        <td class="TableContent" width="15%"><?=$ov?></td>
