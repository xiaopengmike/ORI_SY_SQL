...
$selectParam = array(
    'is_used' => '1',
    'type' => 'crm_process_sort,crm_process_type,yes_or_no'
);
...
if (array_key_exists('material_name', $param) && $param['material_name'] != "") {
    $whereStr .= " and k.material_name like '%" . trim($param['material_name']) . "%' ";
}

if (array_key_exists('create_user_name', $param) && $param['create_user_name'] != "") {
    $whereStr .= " and u.user_name like '%" . $param['create_user_name'] . "%' ";
}
if (array_key_exists('m_specification_model', $param) && $param['m_specification_model'] != "") {
    $whereStr .= " and k.m_specification_model = '" . $param['m_specification_model'] . "' ";
}
...
if (array_key_exists('status', $param) && $param['status'] != "") {
    ...
    if(in_array($param['status'],array("YP","YY","YE","YN"))){
        $whereStr .= $param['status']=="YN"?" ":" and k.status in('R','Y')";
    }else{
        $whereStr .= " and k.status = '" . $param['status'] . "' ";
    }
    //索样确认
    if($param['status']=="YP"){
        $whereStr .= " and k.close_status = 'P' ";
    }
    //料号申请
    if($param['status']=="YY"){
        $whereStr .= " and k.close_status = 'Y' ";
    }
    //结束
    if($param['status']=="YE"){
        $whereStr .= " and k.close_status = 'E' ";
    }
    //已作废
    if($param['status']=="YN"){
        $whereStr .= " and k.close_status = 'N' ";
    }
...
while ($item = mysql_fetch_assoc($res)) {
    if($item[status]=="P"){
        $sql2="select o.work_flow,u.user_name from inhe_flow_opinion o left join user u on u.user_id=o.approve_user where o.form_id='$item[form_id]' and o.status='U' limit 1";
        if ($item2 = mysql_fetch_assoc(exequery(TD::conn(), $sql2))) {
            $item[work_flow]=$item2[work_flow];
            $item[approve_user_name]=$item2[user_name];
        }
    }
...
if ($type == 'trial_sample_request') $url = 'trial_add.php';
?>
...
<table id="tab1" class="layui-table table-auto" style="width: 98%;margin:auto;">
    <tr class="layui-bg-head layui-bg-gray" id="tabHead">
        ...
        <th nowrap style="text-align: center;">所用项目</th>
        <th nowrap style="text-align: center;">物料名称</th>
        <!-- <th nowrap style="text-align: center;">料号是否启用</th> -->
        <th nowrap style="text-align: center;">料号</th>
        ...
        <th nowrap style="text-align: center;">当前进度</th>
        ...
        <?php else : ?>
            ...
            foreach ($result as $k => $val) : $line++; ?>
                <tr class="TableLine<?= $line % 2 == 0 ? 1 : 2 ?>">
                    ...
                    <!-- <td align="center"><?= $yes_or_no[$val["pm_status"]] ?></td> -->
                    <td align="center"><?= $val["part_number"] ?></td>
                    <td align="center"><?= $val["deptName"] . ' ' . $val["createUserName"] ?></td>
                    <td align="center"><?= $val["write_time"] ?></td>
                    <td align="center">
                    <?php if ($val["close_status"] == 'P'&&($val["status"] == 'Y'||$val["status"] == 'R')) : ?>
                            <font class="<?= SystemEnum::$STATUS_CSS['P'] ?>"><?= '索样确认' ?></font>
                        <?elseif($val["close_status"] == 'Y'&&($val["status"] == 'Y'||$val["status"] == 'R')):?>
                            <font class="<?= SystemEnum::$STATUS_CSS['Y'] ?>" ><?= '申请料号' ?></font>
                        <?elseif($val["close_status"] == 'N'):?>
                            <font class="<?= SystemEnum::$STATUS_CSS['N'] ?>"><?= '流程废弃' ?></font>
                        <?else:?>
                            <font class="<?= SystemEnum::$STATUS_CSS[$val["status"]] ?>"><?= SystemEnum::$STATUS[$val["status"]] ?></font>
                        <?php endif; ?>
                        
                    </td>
                    <td align="center"><?=$val[work_flow]?$val[work_flow]."：".$val[approve_user_name]:""?></td>
                    <td align="center">
                        ...
                        <?php endif; ?>
                        <?php if (($userId == $val['approve_user'])&& ($val["status"] == 'R' )) : ?>
                            <a href="javascript:void(0);" class="newColor" onclick="pageOperate('detail.php?id=<?= $val[form_id] ?>&operate=approve&step=<?=$val[step]?>','detail');return false;">备案</a>&nbsp;
                        <?php endif; ?>
                        <?php if (($userId == $val['user_id'] || $systemAdmin) && ($val["status"] == 'T' || ($val["status"] == 'N'&&$val['step']==1))) : ?>
                        ...
                        <?php if (($userId == $val['purchasing_engineer']|| $systemAdmin) &&(($val["status"] == 'R'||$val["status"] == 'Y')||$val["status"] == 'E')) : ?>
                            <a href="javascript:void(0);" class="newColor" onclick="pageOperate('./sample_comfirm/list.php?request_id=<?= $val[form_id] ?>','detail');return false;">索样确认</a>&nbsp;
                        <?php endif; ?>
                        <?php if (($userId == $val['user_id'] || $systemAdmin)&&empty($val[part_number]) &&$val["close_status"] != 'Y'&&($val["status"] == 'R'||$val["status"] == 'Y')) : ?>
                            <a href="javascript:void(0);" class="newColor" onclick="returnData('./action.php?action=updatePurchasePartNumeber&form_id=<?= $val[form_id] ?>','<?= $val[part_number] ?>','<?= $val[pm_status] ?>');return false;">申请料号</a>&nbsp;
                        <?php endif; ?>
                        <?php if (($userId == $val['user_id'] || $systemAdmin) &&$val["close_status"] == 'Y'&&($val["status"] == 'R'||$val["status"] == 'Y')) : ?>
                            <a href="javascript:void(0);" class="newColor" onclick="returnData('./action.php?action=updatePartNumeber&form_id=<?= $val[form_id] ?>','<?= $val[part_number] ?>','<?= $val[pm_status] ?>');return false;">申请料号</a>&nbsp;
                        <?php endif; ?>
