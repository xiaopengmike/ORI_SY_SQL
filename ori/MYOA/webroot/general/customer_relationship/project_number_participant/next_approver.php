...
if ($param['actionType'] == 'Back' || $ifAgree == FormModel::DISAGREE) {
    ...
    $sql = "select * from inhe_number_participant k left join department d on k.organ_id=d.dept_id 
    where form_id = '" . $param['form_id'] . "' and exists(select * from inhe_contract_review where project_number=k.project_number and  user_bu=k.user_bu and create_user_bu=d.user_bu ) ";
    if($approveInfo[step]=='2'&&($approverArr[0][approver]==$userId||mysql_fetch_assoc(exequery(TD::conn(), $sql)))){
    //if($approveInfo[step]=='2'&&$approverArr[0][approver]==$userId){
...
if (empty($approverArr) || count($approverArr) == 0) {
    ...
    $sql = "update inhe_number_participant set status = 'Y' where form_id = '" . $param['form_id'] . "' ";
    exequery(TD::conn(), $sql);
    $processModel->updateChangeVersion('inhe_number_participant',$param['form_id']);
    header("Location: ../common/list/finish_page.php");
    exit;
}

$param['tableName'] = 'inhe_number_participant';
        ...
        <div class="layui-form-item">
            ...
            <?php foreach ($approverArr as $app) : ?>
                <div class="layui-input-block">
                    <input type="radio" name="step" data-dept_id="<?=$data['organ_id']?>" value="<?= $app['step'] ?>" title="<?= $app['role'] ?>" data-type="<?= $app['operate'] ?>" data-approver="<?= $app['approver'] ?>" lay-filter="step" />
                </div>
            <?php endforeach; ?>
        </div>
