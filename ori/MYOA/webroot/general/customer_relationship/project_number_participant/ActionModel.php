...
class ActionModel
{
    public $systemId = "project_number_participant";
    ...
    public function addMain($param)
    {
        ...
        $mainSql = "insert into inhe_number_participant (
            form_id,
            title,
            dept_belong,
            project_code,
            ...
            "('" . $param['form_id'] . "', '" . $param['title'] . "', '" . $param['dept_name'] . "', '" . $param['project_name'] . "', '" . $param['project_number'] .
            "', '" . $param['customer_name'] . "','" . $param['leader_id'] . "', '" . $param['leader'] . "', '" . $param['leader_proportion'] .
            "', '" . $param['status'] . "','" . $deptId . "', '" . $userId .
            "', '" . $time . "','" . $param['type'] . "','" . $param['remark'] . "','" . $param['user_bu'] . "')";
    ...
    public function addDetail($param)
    {
        ...
        for ($i = 1; $i <= $param['data_count']; $i++) {
            $str = $i == 0 ? '' : $i;
            $sql = "insert into inhe_number_participant_item (
                form_id,
                serial_number,
                user_id,
                user_name,
    ...
    public function queryMain($param)
    {
        $result = array();
        $strWhere = " where 1=1 ";
        $userField = FormModel::USER_ID;
        $deptField = FormModel::ORGAN_ID;
        ...
        // 主表名称
        if (array_key_exists("mainTable", $param) && $param['mainTable'] != "") {
            $mainTable = $param['mainTable'];
        }
        if (array_key_exists("user_field", $param) && $param['user_field'] != "") {
            $userField = $param['user_field'];
        }
        if (array_key_exists("dept_field", $param) && $param['dept_field'] != "") {
            $deptField = $param['dept_field'];
        }
        $orderBy = "";
        // 排序方式
        ...
        $sql = " select k.*,u.user_name createUserName, d.dept_name deptName from $mainTable k " .
            " left join user u on u.user_id = k." . $userField .
            " left join department d on d.dept_id = k." . $deptField . $strWhere . $orderBy;
    ...
    public function queryDetail($param)
    {
        ...
        // 主表名称
        if (array_key_exists("detailTable", $param) && $param['detailTable'] != "") {
            $detailTable = $param['detailTable'];
        }
        // 排序方式
        if (array_key_exists("sortField", $param) && $param['sortField'] != "") {
            $orderBy .= " order by " . $param['sortField'];
        ...
        $sql = " select * from $detailTable k " . $strWhere . $orderBy;
        $res = exequery(TD::conn(), $sql);
    ...
    public function updateMainData($param)
    {
        ...
        $tableName = 'inhe_number_participant';
    ...
    public function updateDetailData($param)
    {
        ...
        $condition = array('form_id' => $param['form_id']);
        $tableName = 'inhe_number_participant_item';
    ...
    public function delete()
    {
        ...
        $sql = " delete from inhe_number_participant " . $where;
        $re = exequery(TD::conn(), $sql);

        $sql = " delete from inhe_number_participant_item where form_id = '" . $param['id'] . "' ";
        $res = exequery(TD::conn(), $sql);
