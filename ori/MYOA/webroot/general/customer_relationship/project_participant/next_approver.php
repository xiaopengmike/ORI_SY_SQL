...
if ($param['actionType'] == 'Back' || $ifAgree == FormModel::DISAGREE) {
    ...
    $sql = "select * from inhe_project_participant k left join department d on k.organ_id=d.dept_id 
    where form_id = '" . $param['form_id'] . "' and exists(select * from inhe_project_main where project_code=k.project_code and  user_bu=k.user_bu and create_user_bu=d.user_bu ) ";
...
if (empty($approverArr) || count($approverArr) == 0) {
    ...
    $sql = "update inhe_project_participant set status = 'Y' where form_id = '" . $param['form_id'] . "' ";
    exequery(TD::conn(), $sql);
    $processModel->updateChangeVersion('inhe_project_participant',$param['form_id']);
    header("Location: ../common/list/finish_page.php");
    exit;
}

$param['tableName'] = 'inhe_project_participant';
