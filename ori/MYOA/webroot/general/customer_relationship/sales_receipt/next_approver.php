...
if (empty($approverArr) || count($approverArr) == 0) {
    ...
    $sql = "update inhe_sales_receipt set status = 'Y' where form_id = '" . $param['form_id'] . "' ";
...
$param['tableName'] = 'inhe_sales_receipt';
...
<script type="text/javascript">
            ...
            form.on('radio(step)', function(data) {
                ...
                if (approver == 'dept_select') {
                    ...
                    $.post(url, {
                        ...
                        var obj = eval('(' + data + ')');
                        if (type == 'record') {
                            var userNameList = $("#approver_list").val();
                            var userIdList = $("#approver_list").parent().find("[data-type='user_id']").val();
    ...
    function getUserSelect(domId) {
        layui.use(['form', 'layer'], function() {
            ...
            layer.open({
                ...
                maxmin: true,
                offset: '50px',
                content: '../common/list/type_list.php?domId=' + domId,
                moveOut: true
            });
            form.render();
        });
    ...
    function returnUser(userName, userId, domId) {
        var userNameList = $("#" + domId).val();
        var userIdList = $("#" + domId).parent().find("[data-type='user_id']").val();
        userNameList += userName;
        userIdList += userId;
        $("#" + domId).val(userNameList);
        $("#" + domId).parent().find("[data-type='user_id']").val(userIdList); //取userId隐藏栏位赋值
    }
</script>
