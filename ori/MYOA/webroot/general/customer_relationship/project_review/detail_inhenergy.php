...
if ($step=='1'&&$operate=="approve"){
    header("Location: modify_inhenergy.php?id=".$main['form_id']);
    exit;
}
// 获取审核数据
    ...
    <script type="text/javascript">
        $(function() {
            layui.extend({
                ...
                $.projectStarSelect();
                 // 招标项目
               // console.log('<?=$main[is_bidding_project]?>'=='01')
            ...
            <input type="hidden" name="soft_count" id="E" value="<?= count($projectDetail['Electric']) ?>" />
                ...
                <table class="TableBlock" width="100%" align="center">
                    <tr>
                        ...
                        <td class="TableData" width="15%"><?= $main['createUserName'] ?></td>
                        <td class="TableContent" width="15%" align="right">日期：</td>
                        <td class="TableData" width="15%"><?= $main['write_time'] ?></td>
                        <td class="TableContent" >是否招标项目：</td>
                        <td class="TableData" >
                        <?= $main['is_bidding_project']=='01'?'是':'否' ?>
                        <input type="hidden" name="is_bidding_project" value="<?= $main['is_bidding_project'] ?>">
                        </td>
                    </tr>
                    <tr>
                        <td colspan="8">
                            <?php include_once("detail/inhenergy/customer_info.php"); ?>
                            <?php include_once("detail/inhenergy/project_situation.php"); ?>
                            <?php include_once("detail/inhenergy/bidding_strategy.php"); ?>
                            
                            <?php
                                if ($main['status']=="Y"&&$operate=='detail') {
                                    include_once("../approve_opinion.php");
                                }else{
                                   include_once("detail/inhenergy/approve_opinion.php");
                                }
                            ?>
                            <?php
