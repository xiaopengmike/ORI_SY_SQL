...
if (($main['user_bu']=='INHENERGY'||$main['user_bu']=='HKNERGY')&&!empty($main['is_bidding_project'])){
    header("Location: modify_inhenergy.php?".$_SERVER['QUERY_STRING']);
    exit;
}
if ($main['user_bu']=='WITLINK'){
    header("Location: modify_witlink.php?".$_SERVER['QUERY_STRING']);
    exit;
}
// 获取审核数据
$dataModel = new DataModel();
    ...
    <script type="text/javascript">
        $(function() {
            layui.extend({
                ...
                form.render();
                var changeStar = <?= empty($main['change_star'])?0:$main['change_star']?>;
                if(changeStar==1){
    ...
    <div style="width: 100%;height:auto;margin:auto;">
        <form id="mainForm" class="layui-form">
            ...
            <div class="weadmin-body" style="width:80%;margin:1% 9%;">
                ...
                <input type="hidden"  name="change_star" value="<?=$main['change_star']?>" />
                ...
                <input type="hidden" id="type" name="type" value="project_review<?= $isChange ?>" />
                ...
                <table width="100%" align="center">
                    <tr>
                        <td align="center">
                            <img alt="" src="../../../../../images/<?= $main['user_bu'] ?>/project_review<?= $isChange ?>_<?= $main['user_bu'] ?>.jpg" style="width: 34%; height: auto;" title="" />
                        </td>
                    </tr>
                    <tr>
                        <td>
                ...
                <table class="TableBlock" width="100%" align="center">
                    <tr>
                        <td class="TableContent" width="15%" align="right">部门：</td>
                        <td class="TableData" width="18%"><?= $userInfo['deptName'] ?></td>
                        <td class="TableContent" width="15%" align="right">制单人：</td>
                        <td class="TableData" width="18%"><?= $userInfo['USER_NAME'] ?></td>
                        <td class="TableContent" width="15%" align="right">日期：</td>
                        <td class="TableData"><?= $currentTime ?></td>
                    </tr>
                    <tr>
                        <td colspan="6">
                            <?php include_once("change/customer_info.php"); ?>
                            <?php include_once("change/project_situation.php"); ?>
                            <?php include_once("change/bidding_strategy.php"); ?>
                        </td>
                    </tr>
                </table>
            </div>
