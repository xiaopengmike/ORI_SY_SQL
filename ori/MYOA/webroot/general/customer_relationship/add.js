    ...
    jQuery.extend({
        ...
        initTextarea: function () {
            ...
            layui.use(['form'], function () {
                form= layui.form;
                form.verify({
                    str_length:function(value, item){
                        var min = item.getAttribute('lay-min');
                        var name = item.getAttribute('lay-name');
                        if(min && value.length < min){
                            return name+'内容不能小于'+min+'个字符的长度';
                        }
                        var max = item.getAttribute('lay-max');
                        if(max && value.length > max){
                            return name+'内容不能大于'+max+'个字符的长度';
                        }
                    },
                    integer: [/^([1-9][0-9]*)$/, "只能填写整数"],
                });
                //form.render();
            });
        },
        tips:function (){
            $(".tips").bind("click",function(){
                var dictId = $(this).attr('tips_class');
                $(this).addClass(dictId);
                var text = $(this).attr('tips_msg')
                layerTipIndex = layer.tips("<span style='color:black'>"+text+"</span>", '.'+dictId, {
                    tips: [1, '#f5dd11'],
                    time: 0
                });
            })
        ...
        formSubmit: function () {
            layui.use(['form', 'layer'], function () {
                ...
                form.on('submit(submit)', function (data) {
                    ...
                    // 单击之后提交按钮不可选,防止重复提交
                    if (actionType == 'Submit' ||actionType == 'Save' ||actionType == 'Save' || actionType == 'Change'){
