...
$selectParam = array(
    ...
    'type' => 'project_star,check_opinion,yes_or_no,inspection_type,display_mode,production_mode,auto_flow_user,
            test_report,packed_in_sequence,product_brand,product_detail_type,company,SGC_TYPE,license_product,license_type'
);
...
$main = $data['main'];
if($main['type']=="rd_production_order"){
    header("Location: ./rd_change.php?id=".$id);
}
if($main['type']=="trial_production_order"){
    header("Location: ./trial_change.php?id=".$id);
}
$attachObj = $attachModel->getAttachById($id);
//存在变更，则取上次变更信息继续变更
if($main['version']>0){
    $item=$dataModel->getLastChange($main['form_id'],$main['version'],'production_order_change');
    if($item['form_id']){
    ...
    <div style="width: 100%;height:auto;margin:auto;">
        <form id="mainForm" class="layui-form">
            ...
            <div class="weadmin-body" style="width:90%;margin:1% 4%;">
                ...
                <input type="hidden" id="type" name="type" value="production_order_change" />
