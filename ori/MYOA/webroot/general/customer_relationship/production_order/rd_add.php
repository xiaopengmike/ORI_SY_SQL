...
$selectParam = array(
    'is_used' => '1', // 可用参数
    'type' => 'yes_or_no,inspection_type,display_mode,production_mode,test_report,
...
<head>
    ...
    <script language="Javascript" type="text/javascript" src="../add.js?v=20240913"></script>
    ...
    <script type="text/javascript">
        $(function() {
            layui.extend({
                ...
                $.formSubmit();
                lineInit();
                $("select[name=form_type").val('<?=$form_type?>');
                form.on('select(form_type)', function(data) {
                    var formType = $("select[name=form_type").val();
                    var url = "../../../../../images/<?= $company ?>/trial_production_order_<?= $company ?>.jpg";
                    if (formType == '02') {
                        url = "../../../../../images/<?= $company ?>/stock_production_order_<?= $company ?>.jpg";
                    }
                    // if (data.value&&data.value!='01'&&data.value!='02') {
                    //     url = "../../../../../images/<?= $main['user_bu'] ?>/"+data.value+"_production_order_<?= $main['user_bu'] ?>.jpg";
                    // }
                    if (formType == '03') {
                        url = "../../../../../images/<?= $company ?>/Replenishment_production_notice.jpg";
                    }
    ...
    <div style="width: 100%;height:auto;margin:auto;">
        <form id="mainForm" class="layui-form" lay-filter="mainForm">
            ...
            <div class="weadmin-body" style="width:90%;margin:1% 4%;">
                ...
                <input type="hidden" id="type" name="type" value="rd_production_order" />
                ...
                <table class="TableBlock" width="100%" align="center">
                    <tr>
                        <td colspan="6">
                            <?php include_once("trial_add/customer_info.php"); ?>
                            <?php include_once("trial_add/order_detail.php"); ?>
                        </td>
                    </tr>
                </table>
            </div>
