...
<table id="tab2" width="100%" style="text-align: left;">
    ...
    <tr>
        <td class="TableHeader" colspan="<?= count($detailItem['meter_one']) + 1 ?>" style="text-align:center"><?=$selectArr['product_detail_type'][0][paras_desc]?>
    ...
    <tr align="left" id="addMeter">
        <td class="TableData" colspan="15">
            <input value="添加" type=button class="layui-btn layui-btn-oa layui-btn-sm" onclick="addLine('addMeter');return false;" />
        </td>
    </tr>
</table>
<?endif;?>
...
<? $line_num=1; foreach($selectArr['product_detail_type'] as $product_type):?>
    <?php  $check_company=$company=='WITLINK'?'SOFT':$company;?>
    <?if(!in_array($company,array('GRIDINHE','IMCINHE','INHENERGY','HKNERGY'))&&$type=="rd_production_order"&&(!in_array(strtoupper($product_type['paras_value']),array($check_company)))&&(!in_array($product_type['paras_value'],array('Other')))){continue;}?>
...
<table id="tab3" width="100%" style="text-align: left;">
    ...
    <tr align="left" id="add<?=$product_type[paras_value]?>">
        <td class="TableData" colspan="<?= count($detailItem[$detailItemType]) ?>">
            <input value="添加" type=button class="layui-btn layui-btn-oa layui-btn-sm" onclick="addLine('add<?=$product_type[paras_value]?>');return false;" />
        </td>
    </tr>
</table>
<?php endforeach;?>
...
<tr>
    ...
    <tr>
        ...
        <td class="TableData" colspan="3" align="left">
            <textarea class="layui-textarea TextDetails" id="other_explain" name="other_explain" placeholder="请填写创建此类生产通知单理由" <?if($main[form_type]=="03"||$main[form_type]=="04"):?>lay-verify="required" <?endif;?>><?= $main['other_explain'] ?></textarea>
        </td>
    </tr>
    <tr>
