...
class ActionModel
{
    ...
    public function queryMain($param)
    {
        ...
        $sql = " select k.*,u.user_name createUserName,d.dept_name deptName from inhe_lieu_receive k " .
    ...
    public function submit()
    {
        ...
        if (array_key_exists('action', $result)) {
            ...
            $excipients_id=$this->verifyExist($result['param'],'excipients_detail_id');
            if($result['param']["addNonLieuCount"]>0&&$excipients_id!=false){
                $result = array(
                    'code' => '0',
                    'status' => 'failed',
                    'msg' => '存在非待处理物料,辅料编号：'.$excipients_id,
                    'data' => '存在非待处理物料,辅料编号：'.$excipients_id
                );
                return $result;
            }
    ...
    {
        $mainSql = "insert into inhe_lieu_receive (title, 
        form_id, 
        other_explain, 
        organ_id, 
        user_id, 
        ...
        excipients_form_id
            ...
            $param['user_bu'].  "', '" .$param['excipients_form_id'] . "')";
    ...
    public function addNonMeter($param)
    {
        ...
        for ($i = 1; $i <= $param['addNonLieuCount']; $i++) {
            $str = "_NonLieu".$i;
            if (!empty($param['non_lieu_name' . $str])) {
                $sql = "insert into inhe_lieureceive_detail (form_id, 
                non_lieu_serial_number, 
                non_lieu_name, 
                non_lieu_specification, 
                non_lieu_model, 
                non_lieu_number, 
                non_lieu_attachment,
                non_lieu_excipients_id
                    ) values" .
                    "('" . $param['form_id'] . "', '" . $param['non_lieu_serial_number' . $str] . "', '" . $param['non_lieu_name' . $str] . "', '" .
                    $param['non_lieu_specification' . $str] . "','" . $param['non_lieu_model' . $str] . "', '" . $param['non_lieu_number' . $str] . "','" .
                    $param['non_lieu_attachment' . $str]. "','" .$param['non_lieu_excipients_id' . $str] . "')";
    ...
    public function updateNoneMeter($param)
    {
        ...
        $batchField = array(
            'non_lieu_serial_number' => 'non_lieu_serial_number',
            'non_lieu_name' => 'non_lieu_name',
            'non_lieu_specification' => 'non_lieu_specification',
            'non_lieu_model' => 'non_lieu_model',
            'non_lieu_number' => 'non_lieu_number',
            'non_lieu_attachment' => 'non_lieu_attachment',
            'non_lieu_excipients_id' => 'non_lieu_excipients_id',
        );
        
        $typeArr[]="NonLieu";
        $countArr["NonLieu"]=$param["addNonLieuCount"];
        $condition = array('form_id' => $param['form_id']);
        $tableName = 'inhe_lieureceive_detail';
        $result = $form->updateBatchDetailByType($normalField, $batchField, $param, $condition, $tableName, $typeArr, $countArr);
        $sql = " delete from $tableName where form_id = '" . $param['form_id'] . "' and (non_lieu_name = '' or non_lieu_name is null) ";
        exequery(TD::conn(), $sql);
        return $result;
    }
    ...
    public function changeNoneMeter($param)
    {
        ...
        $deleteSql = "delete from inhe_lieureceive_detail where non_lieu_name = '' and form_id = '" . $param['form_id'] . "' ";
    ...
    public function delete()
    {
        ...
        $sql = " delete from inhe_lieu_receive " . $where;
        $re = exequery(TD::conn(), $sql);

        $sql = " delete from inhe_lieureceive_detail  where form_id = '" . $param['id'] . "' ";
        $res = exequery(TD::conn(), $sql);
     ...
     */
    public function verifyExist($param,$type='excipients_detail_id'){
        switch($type){
            case 'excipients_detail_id':
                for ($i = 1; $i <= $param['addNonLieuCount']; $i++) {
                    $str = "_NonLieu".$i;
                    if (!empty($param['non_lieu_excipients_id' . $str])) {
                        $tempSql = " select a.non_lieu_excipients_id from inhe_lieureceive_detail a,inhe_lieu_receive b where a.form_id=b.form_id and a.non_lieu_excipients_id='".$param['non_lieu_excipients_id' . $str]."' and b.status in('R','Y','P')";
                        $res=exequery(TD::conn(), $tempSql);
                        if($item = mysql_fetch_assoc($res)){
                            return $item[non_lieu_excipients_id];
                            break;
                        }
                    }
                }
