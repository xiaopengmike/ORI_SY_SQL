...
class ActionModel
{
    ...
    public function queryMain($param)
    {
        ...
        $sql = " select m.*,d.dept_name realDeptName from inhe_project_schedule m " .
    ...
    public function submit()
    {
        ...
        if(!$this->verifyProjectCodeExist($param['project_code'])){
            $result = array(
                'code' => '0',
                'status' => 'failed',
                'msg' => '项目代号不存在，请填写已经存在立项记录的项目代号！',
                'data' => '项目代号不存在，请填写已经存在立项记录的项目代号！'
            );
            return $result;
        }
        $form = new FormModel();
    ...
    public function addMain($param)
    {
        ...
        //$where = " where not exists (select * from inhe_project_schedule where form_id='".$param['form_id']."')";
        //$limit = " limit 1";
        $sql = "insert into inhe_project_schedule (title, 
            form_id,
            project_code,
            progress_nodes,
            region,
            ...
            type,
            user_bu)" .
            " values( '" . $param['title'] . "', '" . $param['form_id'] . "', '" . $param['project_code'] . "', '" . $param['progress_nodes'] . "', '" . $param['region'] . "', '" .
            $param['region_id']. "','" .$param['nodes_desc']. "','" . $param['project_number']. "','" . $userName. "','" .
        ...
        $this->addDetail($param['project_number'],$param['form_id'],$insert_id);
        return true;
    }
    public function addDetail($project_number,$form_id,$schedule_id)
    {
        if(empty($project_number)){
            return false;
        }
        $project_number=explode(",", $project_number);
        $insetSql="";
        foreach($project_number as $k=>$v){
            if(empty($v)){
                continue;
            }
            $v=trim($v);
            $insetSql.="('$form_id','$schedule_id','$v'),";
        }
        $sql = "insert into inhe_project_schedule_detail (form_id, schedule_id,project_number)values" ;
        $insetSql=rtrim($insetSql,",");
        if($insetSql){
            exequery(TD::conn(), $sql.$insetSql);
        }
    ...
    public function verifyDataExist($formId)
    {
        $flag = false;
        if (isset($formId)) {
            $sql = " select * from inhe_project_schedule where form_id = '$formId' ";
            $res = exequery(TD::conn(), $sql);
    ...
    public function verifyProjectCodeExist($project_code)
    {
        $flag = false;
        if (isset($project_code)) {
            $sql = " select * from inhe_project_main where project_code = '$project_code' ";
            $res = exequery(TD::conn(), $sql);
            $count = mysql_num_rows($res);
            if ($count > 0) {
                $flag = true;
    ...
    public function updateMainData($param)
    {
        ...
        $paramInsert = array(
            ...
            'progress_nodes' => $param['progress_nodes'],
            'project_number' => $param['project_number'],
            'update_time' => date('Y-m-d H:i:s'),
            'nodes_desc' => $param['nodes_desc'],
            'status' => $param['status'],
            'type' => $param['type'],
        ...
        $tableName = 'inhe_project_schedule';
        $result = $form->updateMainData($paramInsert, $condition, $tableName, array(), '');
        $sql = " delete from inhe_project_schedule_detail where schedule_id='$param[schedule_id]'";
        $re = exequery(TD::conn(), $sql);
        $this->addDetail($param['project_number'],$param['form_id'],$param[schedule_id]);
        return $result;
    }
    ...
    public function Forbidden()
    {
        ...
        $main=$this->queryMain($param);
        $status=$main["status"]=="Y"?"T":"Y";
        $sql = " update  inhe_project_schedule set status='$status' " . $where;
        $re = exequery(TD::conn(), $sql);
    ...
    public function delete()
    {
        ...
        $sql = " delete from inhe_project_schedule " . $where;
        $re = exequery(TD::conn(), $sql);
        $sql = " delete from inhe_project_schedule_detail where schedule_id='$param[id]'";
        $re = exequery(TD::conn(), $sql);
