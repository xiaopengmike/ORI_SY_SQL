...
function createLine(type) {
    ...
    if (type == 'addContact') {
        ...
        if($("#user_bu").val()=="INHENERGY"||$("#user_bu").val()=="HKNERGY"){
            input_class="mandatory";
            input_attr='lay-verify="required"';
        }
        html = '<tr>';
        html += '<td  class="TableData"><input type="text" class="layui-input '+input_class+'" '+input_attr+' id="contacts_name" name="contacts_name' + contactNum + '" /></td>';
        html += '<td  class="TableData"><input type="text" class="layui-input '+input_class+'" id="phone_one" name="phone_one' + contactNum + '" /></td>';
        html += '<td  class="TableData"><input type="text" class="layui-input " id="phone_two" name="phone_two' + contactNum + '" /></td>';
        html += '<td  class="TableData"><input type="text" class="layui-input '+input_class+'" id="email" name="email' + contactNum + '" /></td>';
...
function shareCustomer(id) {
    if(id == "" || id == null || id == undefined || id == "NaN"){
        ...
        var url = "../customer_share/share.php?id=" + id;
        pageOperate(url, 'add');
    ...
    layui.use(['form', 'layer'], function () {
        ...
        $.post(url, {
            ...
            if (reJson.code == "200") {
                if (reJson.data) {
                    $("#customer_name").val('');
                    var msg=$("#LANG").val()=="EN"?"Customer already exists!":'客户已存在!';
                    layer.alert(msg, {
                        icon: 5
                    });
                }
                ...
                var msg=$("#LANG").val()=="EN"?"Failed to obtain customer!":'获取客户失败!';
                layer.alert(msg, {
                    icon: 5
                });
            }
