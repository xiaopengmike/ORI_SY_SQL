...
<div style="text-align: right;width: 98%;margin:auto;">
    <?php if (strpos($type, '_change') == false) { ?>
        <div style="float: left;">
            ...
            <div class="dropdown">
                <button class="layui-btn layui-btn-oa layui-btn-sm drop-btn"><?=$LG_CUSTOMER_DATA['申请'][$_COOKIE['LANG']]?></button>
            ...
            <!-- <button class="layui-btn layui-btn-oa layui-btn-sm" onclick="pageOperate('record_import/pre_import.php','smallModel');return false;" ><?=$LG_CUSTOMER_DATA['导入'][$_COOKIE['LANG']]?></button> -->
            <?php endif; ?>
            <?php if ($userBu!="INHENERGY") : ?>
            <button style='margin-left:0px' class="layui-btn layui-btn-oa layui-btn-sm" onclick="$.startChangeForm();return false;"><?=$LG_CUSTOMER_DATA['变更'][$_COOKIE['LANG']]?></button>
            <?php endif; ?>
            <?php if ($systemAdmin) : ?>
...
<table id="tab1" class="layui-table table-auto" style="width: 98%;margin:auto;">
    <tr class="layui-bg-head layui-bg-gray" id="tabHead">
        ...
        <th nowrap style="text-align: center;">
            <span><?=$LG_CUSTOMER_DATA['制单时间'][$_COOKIE['LANG']]?></span>
        ...
        <th nowrap style="text-align: center;"> 
        <span> <?=$LG_CUSTOMER_DATA['跟进距今天数'][$_COOKIE['LANG']]?></span>
                ...
                <tr class="TableLine<?= $line % 2 == 0 ? 1 : 2 ?>">
                    ...
                    <td align="center" nowrap>
                        ...
                        <?php if ($userBu=="INHENERGY" && strpos($type, '_change') == false && ($val["status"] == 'R' || $val["status"] == 'Y')) : ?>
                            <a href="javascript:void(0);" class="newColor" onclick="pageOperate('change.php?id=<?= $val[form_id] ?>','detail');return false;"><?=$LG_CUSTOMER_DATA['修改'][$_COOKIE['LANG']]?></a>&nbsp;
                        <?php endif; ?>
                        <?php if (($val['user_bu']=="INHENERGY"||$val['user_bu']=="HKNERGY") && strpos($type, '_change') == false && ($val["status"] == 'R' || $val["status"] == 'Y')) : ?>
                            <a href="javascript:void(0);" class="newColor" onclick="pageOperate('flow.php?id=<?= $val[form_id] ?>','detail');return false;"><?=$LG_CUSTOMER_DATA['跟进'][$_COOKIE['LANG']]?></a>&nbsp;
                        <?php endif; ?>
                        
                        <?php if ($systemAdmin&&$userBu=="INHENERGY" && strpos($type, '_change') == false && ($val["status"] == 'R' || $val["status"] == 'Y')) : ?>
                            <!-- <a href="javascript:void(0);" class="newColor" onclick="shareCustomer('<?= $val[form_id] ?>');return false;"><?=$LG_CUSTOMER_DATA['共享'][$_COOKIE['LANG']]?></a>&nbsp; -->
                        <?php endif; ?>
                    </td>
                </tr>
            <?php endforeach; ?>
