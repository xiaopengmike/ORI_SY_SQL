...
$HTML_PAGE_TITLE = _($LG_CUSTOMER_DATA['客户信息列表'][$_COOKIE['LANG']]);
...
<script>
    ...
    function test(){
        ...
        if(!ifEmpty($("#flow_date").val())&&!pattern.test($("#flow_date").val())){
            alert($LG_CUSTOMER_DATA['跟进距今天数只能填写数字'][$_COOKIE['LANG']]);
            return false;
        }
        $.loadList('1');
...
<body class="bodyColorNew">
    <form id="tab" class="layui-form">
        ...
        <input type="hidden" name="data_flow" id="data_flow" class="layui-input" value="<?=$data_flow?>" placeholder="请输入工号">
        <input type="hidden" name="order_will" id="order_will" class="layui-input" value="<?=$order_will?>" placeholder="请输入工号">
        <table style="margin: 1%">
            <tr>
                ...
                <td align="right"><?=$LG_CUSTOMER_DATA['简称'][$_COOKIE['LANG']]?></td>
                <td>
                ...
                <tr>
                    <td align="right"><?=$LG_CUSTOMER_DATA['大洲'][$_COOKIE['LANG']]?></td>
                    <td>
                    ...
                    <td align="right"><?=$LG_CUSTOMER_DATA['国家'][$_COOKIE['LANG']]?></td>
                    <td>
                    ...
                    <td align="right"><?=$LG_CUSTOMER_DATA['联系人'][$_COOKIE['LANG']]?></td>
                    <td>
                        <input type="text" name="contact" id="contact" class="layui-input" placeholder="<?=$LG_CUSTOMER_DATA['姓名'][$_COOKIE['LANG']]?>/<?=$LG_CUSTOMER_DATA['电话'][$_COOKIE['LANG']]?>/<?=$LG_CUSTOMER_DATA['邮箱'][$_COOKIE['LANG']]?>">
                    </td>
                    <td align="right"><?=$LG_CUSTOMER_DATA['制单人'][$_COOKIE['LANG']]?></td>
                    <td>
                        <input type="text" name="create_user_name" id="create_user_name" class="layui-input" placeholder="<?=$LG_CUSTOMER_DATA['制单人'][$_COOKIE['LANG']]?>">
                    </td>
                </tr>
                <?endif;?>
                <tr> 
                ...
                <td  align="right"><?=$LG_CUSTOMER_DATA['跟进距今天数'][$_COOKIE['LANG']]?></td>
                <td>
                    <input  type="text" name="flow_date" id="flow_date" class="layui-input"  placeholder="<?=$LG_CUSTOMER_DATA['大于多少天'][$_COOKIE['LANG']]?>">
                </td>
                <td  align="right"><?=$LG_CUSTOMER_DATA['是否有效'][$_COOKIE['LANG']]?></td>
                <td>
                    <div class="layui-input-inline" >
                        <select id="is_vaild" name="is_vaild" >
                            <option value=''><?=$LG_CUSTOMER_DATA['请选择'][$_COOKIE['LANG']]?></option>
                            <option value='01'><?=$LG_CUSTOMER_DATA['是'][$_COOKIE['LANG']]?></option>
                            <option value='02'><?=$LG_CUSTOMER_DATA['否'][$_COOKIE['LANG']]?></option>
                        </select>
                    </div>
                </td>
                <td  align="right"><?=$LG_CUSTOMER_DATA['是否成交订单'][$_COOKIE['LANG']]?></td>
                <td>
                    <div class="layui-input-inline" >
                        <select id="own_orders" name="own_orders" >
                            <option value=''><?=$LG_CUSTOMER_DATA['请选择'][$_COOKIE['LANG']]?></option>
                            <option value='Y'><?=$LG_CUSTOMER_DATA['是'][$_COOKIE['LANG']]?></option>
                            <option value='N'><?=$LG_CUSTOMER_DATA['否'][$_COOKIE['LANG']]?></option>
                        </select>
                    </div>
                </td>

                <td>
                    <button class="layui-btn layui-btn-oa layui-btn-sm" onclick="test();return false;" style="margin-left: 10px;"><?=$LG_CUSTOMER_DATA['查询'][$_COOKIE['LANG']]?></button>
