...
$sql = " select m.*, t.filename applyAttachName, tt.filename bankAttachName, ttt.filename transferAttachName from inhe_authorization_letter m "
    // . "left join inhe_oa_paras pp on pp.type = 'bid_type' and pp.paras_value = m.contract_category "
    . "left join inhe_attachment t on t.type = 'apply_attach' and t.relatedId = m.id "
    . "left join inhe_attachment tt on tt.type = 'bank_attach' and tt.relatedId = m.id "
    . "left join inhe_attachment ttt on ttt.type = 'transfer_attach' and ttt.relatedId = m.id "
    . $where . $orderBy . $pageLimit;
$countSql = " select count(*) total_count from inhe_authorization_letter m ". $where . $orderBy;
// var_export($sql);exit;
...
while ($item = mysql_fetch_assoc($res)) {
    $item['apply_attach'] = $item['apply_attach'] == '' ? $item['applyAttachName'] : $item['apply_attach'];
    $item['bank_attach'] = $item['bank_attach'] == '' ? $item['bankAttachName'] : $item['bank_attach'];
    $item['transfer_attach'] = $item['transfer_attach'] == '' ? $item['transferAttachName'] : $item['transfer_attach'];
