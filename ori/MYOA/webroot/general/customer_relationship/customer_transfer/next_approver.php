...
<?php
$param['tableName'] = 'inhe_customer_transfer';
...
if (empty($approverArr) || count($approverArr) == 0) {
    ...
    $sql = "update inhe_customer_transfer set status = 'Y' where form_id = '" . $param['form_id'] . "' ";
    exequery(TD::conn(), $sql);
    $processModel->updateChangeVersion('inhe_customer_transfer',$param['form_id']);
    
    $sql = "select * from user where user_id = '$data[to_userid]'";
    $userRes=exequery(TD::conn(), $sql);
    $userInfo =  mysql_fetch_assoc($userRes);
    //更新客户归属
    $sql = "update inhe_customer_data set user_id = '$userInfo[USER_ID]',organ_id = '$userInfo[DEPT_ID]' where user_id='$data[from_userid]' and find_in_set(id,'$data[customer_ids]')";
    exequery(TD::conn(), $sql);
    //删除原共享，增加新共享
    $sql = "delete from inhe_customer_share  where user_ids='$data[from_userid]' and find_in_set(customer_id,'$data[customer_ids]')";
    
    exequery(TD::conn(), $sql);
    $sql = "insert into inhe_customer_share(customer_id,customer_name,user_ids,form_id,user_id,create_time)select customer_id,customer_name,'$data[to_userid]' user_ids,form_id,'$data[user_id]' create_user_id,'$time' create_time from inhe_customer_transfer_detail where transfer_type='share' and form_id='$data[form_id]'";
    exequery(TD::conn(), $sql);
