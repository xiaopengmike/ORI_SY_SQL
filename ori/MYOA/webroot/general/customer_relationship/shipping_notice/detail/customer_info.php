<table id="tab1" width="100%" style="text-align: center;">
    ...
    <tr>
        ...
        <td class="TableData">
            <input type="text" name="shipment_time" id="shipment_time" class="layui-input" value="<?= $main['shipment_time'] ?>" />
        </td>
    </tr>
    <tr>
        ...
        <td class="TableData">
            <input type="text" name="currency" id="currency" class="layui-input" value="<?= $currency[$main['currency']] ?>" />
        </td>
        <?endif;?>
    </tr>
    <tr>
        ...
        <td class="TableData">
            <input type="text" name="create_user" id="create_user" class="layui-input" value="<?= $main['createUserName'] == '' ? $main['create_user'] : $main['createUserName'] ?>" />
        </td>
    </tr>
    <?if(!in_array($company,array('INHENERGY','HKNERGY','RETAILNERGY'))):?>
    <tr>
        ...
        <td class="TableData">
            <input type="text" class="layui-input" name="have_entrepot_trade" id="have_entrepot_trade" value="<?= $yes_or_no[$main['have_entrepot_trade']]['paras_desc'] ?>" />
        </td>
        ...
        <td class="TableData">
            <input type="text" name="create_time" id="create_time" class="layui-input" value="<?= $main['write_time'] ?>" />
        </td>
    </tr>
    <tr>
    <td class=" TableContent">产品经理(客服)</td>
            ...
            <input style="width:288px;float:left" placeholder="请点击选择产品经理，流程结束备案到该人员" onClick="SelectUserSingle('','customer_service', 'customer_service_name')" class="layui-input" id="customer_service_name" name="customer_service_name" value="<?= $main['customer_service_name']?>" readonly ></input>
        </td>   
        <td class=" TableContent">客户是否已申请好进口国进口许可？</td>
        <td class="TableData" colspan="">
            <input type="text" class="layui-input" name="import_permit" id="import_permit" value="<?= $yes_or_no[$main['import_permit']]['paras_desc'] ?>" />
        </td>  
    ...
    <?php if ($main['user_bu'] == 'INHEGRID') { ?>
        <tr>
            <td class=" TableContent">通知单类型</td>
            <td class="TableData" align="left" colspan="5">
                <input type="text" name="notice_type" id="notice_type" class="layui-input" value="<?= $notice_type[$main['notice_type']] ?>" />
            </td>
        </tr>
    <?php } ?>
</table>