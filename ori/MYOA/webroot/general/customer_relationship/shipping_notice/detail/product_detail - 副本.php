...
<?if($product['electric']):?>
    <tr>
        <td class="TableHeader" colspan="9">所发电表类产品明细(请选择项目编号，可带出对应的其他栏位信息）</td>
    </tr>
...
<?if($product['soft']):?>
    ...
    <tr>
        <td class="TableHeader" colspan="9">软件类产品明细(含软件、软件相关服务、软件相关产品掌上机、短信猫等)</td>
    </tr>
    <?if($contract[contract_begin_time]):?>
        <tr >
    ...
    <?if($main[user_bu]=="WITLINK"):?>
        ...
        <td>授权产品</td><td>系统版本</td><td>授权生效时间</td><td>授权次数</td><td>授权天数（单位:天）</td><td>授权到期时间</td><td>授权码</td><td  colspan="2"></td>
        </tr>
        <?php $colNum=0;$colSoftNum=0; foreach ($licenseProduct as $dn => $dr) : if ($dn == 0) $dn = ''; ?>
            <?php  ++$colNum;++$colSoftNum;  ?>
...
<?if($product['inhegrid']):?>
    <tr>
        <td class="TableHeader" colspan="9">所发银河电网产品明细</td>
    </tr>
    <tr class="TableContent">
        <td width="6%">序号</td>
        <td width="14%">品名</td>
...
<?if($product['inhenergy']):?>
    <tr>
        <td class="TableHeader" colspan="9">所发银河耐吉产品明细</td>
    </tr>
    <tr class="TableContent">
        <td width="6%">序号</td>
        <td width="14%">品名</td>
...
<?if($product['other']):?>
    <tr>
        <td class="TableHeader" colspan="9">所发其他类产品明细</td>
    </tr>
    <tr class="TableContent">
        <td width="6%">序号</td>
        <td width="14%">品名</td>
...
<table id="pro_detail2" width="100%" style="text-align: center;">
    <tr>
        ...
        <td class="TableData">
            <input type="text" name="total_amount" id="total_amount" class="layui-input" value="<?= $isHide?'':number_format($eleAmount +$inhegridAmount+$inhenergyAmount+ $otherAmount + $softAmount, 4, '.', ''); ?>" />
        </td>
    </tr>
    <tr>
