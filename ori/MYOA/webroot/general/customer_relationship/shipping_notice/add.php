...
$selectParam = array(
    'is_used' => '1', // 可用参数
    'type' => 'project_star,check_opinion,currency,yes_or_no,quote_source,sale_model,inhegrid_notice_type,license_product,license_type'
);
    ...
    <script type="text/javascript">
        ...
        function returnProductInfo(reData, domId) {
            layui.use(['form', 'layer'], function() {
                ...
                for (i; i < length; i++) {
                    if (i == 0) {
                        $("#project_number").val(reData[i].project_number);
                    } else {
                        domId = addLine(typeArr[type]);
                    }
                    ...
                    console.log(type);
                    if(type=="soft"){
                        $("[name='" + domId + "']").parent().parent().find("[id='license_product_" + type + "']").val(reData[i].license_product);
                        $("[name='" + domId + "']").parent().parent().find("[id='license_years_" + type + "']").val(reData[i].license_years);
                        $("[name='" + domId + "']").parent().parent().find("[id='license_type_" + type + "']").val(reData[i].license_type);
                    }
    ...
    <div style="width: 100%;height:auto;margin:auto;">
        <form id="mainForm" class="layui-form">
            ...
            <div class="weadmin-body" style="width:90%;margin:1% 4%;">
                ...
                <input type="hidden" name="tableName" value="inhe_shipping_notice" />
                <input type="hidden" name="flowType" value="fhtz" />
                ...
                <input type="hidden" name="project_number" id="project_number" />
                ...
                <table width="100%" align="center">
                    <tr>
                        <td align="center">
                            <img alt="" src="../../../../../images/<?= $company ?>/shipping_notice_<?= $company ?>.jpg" style="width: 34%; height: auto;" title="" />
                        </td>
                    </tr>
                    <tr>
                        <td>
                ...
                <table class="TableBlock" width="100%" align="center">
                    <tr>
                        <td class="TableContent" width="14%" align="right">立项部门：</td>
                        <td class="TableData" width="19%"><?= $userInfo['deptName'] ?></td>
                        <td class="TableContent" width="14%" align="right">制单人：</td>
                        <td class="TableData" width="19%"><?= $_SESSION['LOGIN_USER_NAME'] ?></td>
                        <td class="TableContent" width="14%" align="right">日期：</td>
