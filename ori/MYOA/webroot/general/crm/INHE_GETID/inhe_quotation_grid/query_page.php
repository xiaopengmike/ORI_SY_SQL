...
//管理员权限查询范围
$adminArr = $model->getAdminDataQuery($userId, "crm_quotation_grid");
echo $adminArr['userBu'].",";
if ($adminArr['isAdmin']&&strpos($adminArr['userBu'].",","'INHEGRID',")!==false) {
    $whereStr .= " ";
} else {
...
$query="select A.*,user.user_name u_name from inhe_crm_quotation_grid A,user WHERE A.USER_ID=user.user_id $whereStr ORDER BY Create_date DESC";
...
<table id="tab1" class="layui-table table-auto">
    <tr class="layui-bg-head layui-bg-gray" id="tabHead">
       <th nowrap style="text-align: center;width: 5%;">序号</th>
        <th nowrap style="text-align: center;width: 10%;">报价单号</th>
