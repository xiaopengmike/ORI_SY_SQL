...
//$query="select * from inhe_crm_sales_confirmation_ihjx ORDER BY Create_date DESC";
$query="select A.*,user.user_name u_name,inhe_agreement_type.agreement_name from inhe_crm_agreement_ihjx A,user,inhe_agreement_type WHERE A.USER_ID=user.user_id and A.agreement_type=inhe_agreement_type.agreement_type and A.USER_ID='" . $userId . "' ORDER BY Create_date DESC";
...
<table id="tab1" class="layui-table table-auto">
    <tr class="layui-bg-head layui-bg-gray" id="tabHead">
       <th nowrap style="text-align: center;width: 5%;">序号</th>
        <th nowrap style="text-align: center;width: 10%;">合作协议号码</th>
        <th nowrap style="text-align: center;width: 10%;">合作方英文全称</th>
        <th nowrap style="text-align: center;width: 10%;">合作协议类型</th>
        <th nowrap style="text-align: center;width: 10%;">授权国家或地区</th>
        <th nowrap style="text-align: center;width: 10%;">项目名称</th>
        <th nowrap style="text-align: center;width: 10%;">创建人</th>
        <th nowrap style="text-align: left;width: 15%;">创建时间</th>
        <th nowrap style="text-align: center;width: 10%;">备注</th>
    </tr>
        ...
        <?php else : ?>
            ...
            foreach ($result as $k => $val) : $line++; ?>
                <tr class="TableLine<?= $line % 2 == 0 ? 1 : 2 ?>">
                    ...
                    <td align="center"><?= $val['Partner_name'] ?></td>
                    <td align="center"><?= $val["agreement_name"] ?></td>
