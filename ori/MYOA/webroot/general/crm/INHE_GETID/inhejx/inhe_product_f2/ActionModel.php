...
class ActionModel
{
    ...
    public function add()
    {
        ...
        $sql3="select max(SC_NO)SC_NO from inhe_crm_product_f2_ihjx where SC_NO LIKE CONCAT('IHJXSAL',DATE_FORMAT(NOW(),'%Y%m%d'),'%')";
        $res3=exequery(TD::conn(),$sql3);
         if($ROW3=mysql_fetch_array($res3)){
	
            $SC_NO='IHJXSAL'.date('Ymd').sprintf("%02d",(substr($ROW3["SC_NO"],15)+1));
	       }
           else
           {            
             $SC_NO='IHJXSAL'.date('Ymd').'01';
           }        
