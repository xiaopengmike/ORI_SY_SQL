...
class ActionModel
{
    ...
    public function add()
    {
        ...
        $sql3="select max(SC_NO)SC_NO from inhe_crm_product_f1_jxih where SC_NO LIKE CONCAT('JXIHSAH',DATE_FORMAT(NOW(),'%Y%m%d'),'%')";
        $res3=exequery(TD::conn(),$sql3);
         if($ROW3=mysql_fetch_array($res3)){
	
            $SC_NO='JXIHSAH'.date('Ymd').sprintf("%02d",(substr($ROW3["SC_NO"],15)+1));
	       }
           else
           {            
             $SC_NO='JXIHSAH'.date('Ymd').'01';
           }        
...
//            "('" . $paras['pm_id'] . "', '" . $userId . "', '" . $userName . "', '" . $paras['Reviewed_ID'] . "', '" . $time . "')";
         ...
         $mainSql = "insert into inhe_crm_product_f1_jxih(SC_NO, PM_ID, USER_NAME, USER_ID, Create_date) values" .
