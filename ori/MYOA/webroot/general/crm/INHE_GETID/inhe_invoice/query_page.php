...
$whereStr = " where 1=1 and A.USER_ID=user.user_id";
...
//管理员权限查询范围
$adminArr = $model->getAdminDataQuery($userId, "crm_invoice");

if ($adminArr['isAdmin']&&in_array("'INHE'",explode(",",$adminArr['userBu']))) {
    $whereStr .= " ";
} else {
...
$query="select A.*,user.user_name u_name from inhe_crm_invoice A,user $whereStr ORDER BY Create_date DESC";
...
<table id="tab1" class="layui-table table-auto">
    <tr class="layui-bg-head layui-bg-gray" id="tabHead">
       <th nowrap style="text-align: center;width: 5%;">序号</th>
        <th nowrap style="text-align: center;width: 10%;">发票号码</th>
