...
class ActionModel
{
    ...
    public function add()
    {
        ...
        $sql3="select max(SC_NO)SC_NO from inhe_crm_agreement_witlink where SC_NO LIKE CONCAT('IHWLAGRT',DATE_FORMAT(NOW(),'%Y%m%d'),'%')";
        $res3=exequery(TD::conn(),$sql3);
         if($ROW3=mysql_fetch_array($res3)){
	
            $SC_NO='IHWLAGRT'.date('Ymd').sprintf("%02d",(substr($ROW3["SC_NO"],16)+1));
	       }
           else
           {            
             $SC_NO='IHWLAGRT'.date('Ymd').'01';
           }        
...
//            "('" . $paras['pm_id'] . "', '" . $userId . "', '" . $userName . "', '" . $paras['Reviewed_ID'] . "', '" . $time . "')";
         ...
         $mainSql = "insert into inhe_crm_agreement_witlink(SC_NO, PM_ID,USER_ID, Create_date,Partner_name,agreement_type,area_name,remark) values" .
           "('" . $SC_NO . "', '" . $paras['PM_ID'] . "','" . $userId . "', '" . $time . "', '" . $paras['Partner_name'] . "', '" . $paras['agreement_type'] . "', '" . $paras['area_name'] . "', '" . $paras['remark'] . "')";
