...
function my_note(CAL_ID)
{
   ...
   _get("note.php", "CAL_ID="+CAL_ID+"&AJAX=1", load_form);
...
function cal_note(CAL_ID)
{
 ...
 myleft=(screen.availWidth-250)/2;
  mytop=(screen.availHeight-200)/2;
  window.open("cal_note.php?CAL_ID="+CAL_ID,"note_win"+CAL_ID,"height=200,width=250,status=0,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable=yes,top="+mytop+",left="+myleft);
}
function aff_note(AFF_ID)
{
  
  myleft=(screen.availWidth-250)/2;
  mytop=(screen.availHeight-200)/2;
  window.open("../info/aff_note_win.php?AFF_ID="+AFF_ID,"note_win"+AFF_ID,"height=200,width=250,status=0,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable=yes,top="+mytop+",left="+myleft);
}
function task_note(TASK_ID)
{
  
  myleft=(screen.availWidth-250)/2;
  mytop=(screen.availHeight-200)/2;
  window.open("../task/note_win.php?TASK_ID="+TASK_ID,"note_win"+TASK_ID,"height=200,width=250,status=0,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable=yes,top="+mytop+",left="+myleft);
}
...
function my_aff_note(AFF_ID,from)
{
   $('title').innerHTML = td_lang.inc.msg_9;//'查看事务'
   ShowDialog('form_div',10);
   _get("aff_note.php", "AFF_ID="+AFF_ID+"&FROM="+from+"&AJAX=1", load_form);
...
function my_task_note(TASK_ID,from,FLAGS)
{
	$('title').innerHTML = td_lang.inc.msg_10;//'查看任务'
  ShowDialog('form_div',10);
  _get("../task/note.php", "TASK_ID="+TASK_ID+"&FROM="+from+"&FLAGS="+FLAGS+"&AJAX=1", load_form);
}
function My_Submit()
{
...
function edit_cal(CAL_ID,DRAG,NEW_TIME)
{
   ...
   ShowDialog('form_div',10);
   _get("modify.php", "CAL_ID="+CAL_ID+"&DRAG="+DRAG+"&NEW_TIME="+NEW_TIME, load_form);
}
...
function del_cal(CAL_ID,from)
{
   if(!window.confirm(td_lang.inc.msg_13))//"删除后将不可恢复,确定要删除吗？"
      return;
   _get("delete.php","CAL_ID="+CAL_ID+"&AJAX=1");
...
function del_cal_pi(CAL_ID,from)
{   
...
{
   var url='../info/delete.php?AFF_ID='+AFF_ID+'&FROM='+from;
...
function del_aff_new_arr(AFF_ID,from)
{
   var url='delete_aff.php?AFF_ID='+AFF_ID+'&FROM='+from;
...
function del_task_new(TASK_ID,from,FLAG)
{ 
   var url='../task/delete.php?TASK_ID='+TASK_ID+'&FROM='+from+'&FLAG='+FLAG;
   if(FLAG==1)
...
function new_arrange(USER_ID, CAL_TIME, TIME_DIFF)
{
   ...
   window.open('index_new.php?USER_ID='+USER_ID+'&CAL_TIME='+CAL_TIME+'&TIME_DIFF='+TIME_DIFF,'oa_sub_window','height=500,width=520,status=0,toolbar=no,menubar=no,location=no,left=300,top=200,scrollbars=yes,resizable=yes');
}
...
function set_status_note(obj,id)
{
   ...
   $('cal_' + id).setAttribute('status', status);
   _get("status.php", "CAL_ID="+id+"&OVER_STATUS="+status,function(req){
   	if(req.responseText == 1){
   		//success
   		_get("note.php", "CAL_ID="+id+"&AJAX=1",load_form);
   	}
  	});
