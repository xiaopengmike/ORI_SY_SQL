...
function ShowNotice(id, url, title, content)
{
    if(jQuery.cookie('hide_notice_' + id) != '1')
    {
        jQuery('#notice_tip').show();
        jQuery('#notice_tip').html('<div style="text-align:center;"><h5>' + title + '</h5></div><div style="line-height:25px;">' + content + '</div><div style="text-align:center;margin:10px 0px;height:25px;line-height:22px;overflow:hidden;"><a href="' + url + '" class="btn-white-b" target="_blank">' + td_lang.inc.msg_146 + '</a>&nbsp;<a href="javascript:;" class="btn-white-b" onclick="jQuery(\'#notice_tip\').hide();jQuery.cookie(\'hide_notice_' + id + '\', 1, {expires:1000, path:\'/\'});">' + td_lang.global.close + '</a></div>');
    }
}
