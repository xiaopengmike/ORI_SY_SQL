...
/*
//打开讨论组窗口 by kw 120928
function ispirit_open_discuss(disc_group_id)
{
	var disc_group_name=jQuery("#disc_group_" + disc_group_id).attr('disc_group_name');
...
(function($){
   ...
   $(document).ready(function(){
      ...
      $("#modules tr.sub-module-head .im-group-op a[discuss_group='1']").live('click', function(){
         parent.openURL('/general/sms/?MAIN_URL=im_group%23discuss_group');
         return false;
      });
      ...
      $("#sub_module_org_0 li[dtnode]").live('mouseenter', mouseenter_org0_node_callback);
      $("#sub_module_org_0 li[dtnode]").live('mouseleave', mouseleave_org0_node_callback);
      $("#sub_module_org_0 li[dtnode] span[op]").live('mouseleave', mouseleave_org0_node_op_callback);
   ...
   $(window).resize(function(){
      ...
      if($(window).height() > $("#sub_tabs").height() + $("#bottom").height())
         $("#body").height($(window).height() - $("#sub_tabs").height() - $("#bottom").height());
...
function xmlDom2TreeNodes(dom, recursive, flag)
{
   ...
   var nodes = [];
   var node = name = id = title = is_org = user_id = dept_id = sex = url = uid_index = client = null;
   for(var i=0; i<dom.length; i++)
   {
      node = dom[i];
      name = node.nodeName;
      id = node.getAttribute("a");
      title = node.getAttribute("b");
      ...
      else
      {
         ...
         url = para_url2 + "?UID=" + id + "&USER_ID=" + user_id + "&DEPT_ID=" + dept_id;
         
         uid_index = jQuery.inArray(id, online_uid_array);
...
//            "tooltip": title,
   ...
   dom = node = name = id = title = is_org = user_id = dept_id = sex = url = uid_index = client = null;
   return nodes;
}
