-- 更新销售排名数据
select * from sales_performance where cycle  = '2020年07月' order by id desc;

update sales_performance set sales_proportion = sales_ranking where cycle  = '2020年07月';

update sales_performance a left join(
select @rd := @rd+1 as ROW_NUM, b.id,b.sales_proportion from (
select 
@rd:=0 row_init,  
	 s.* from sales_performance s where s.cycle = '2020年07月' 
order by sales_proportion*1 desc 
 ) b ) t on t.id = a.id 
set a.sales_ranking = t.ROW_NUM
where a.sales_proportion > 0 and a.cycle  = '2020年07月';


update sales_performance set sales_ranking = CONCAT(FORMAT(sales_ranking*100/8,2),'%')  where cycle  = '2020年07月' and sales_ranking>0 ;

-- 备份回滚数据使用
update sales_performance set sales_proportion = CONCAT(FORMAT(person_actual/com_actual*100,2),'%') where cycle  = '2020年07月';
update sales_performance set sales_ranking = sales_proportion where cycle  = '2020年07月';
...
where a.cycle  = '2020年07月' 

select * from sales_performance_director where cycle  = '2020年07月' order by id desc;
...
where a.cycle  = '2020年07月' 
-- 公共考核分数
select * from inhe_public_kh where cycle = '2020年07月'
-- 一键纠正分数，已有自评分和审核分，未正确计算核定分及总分
select * from inhe_kpikh where ZQ='2020年07月' and final_score = '0.00';
