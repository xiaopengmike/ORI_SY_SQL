update user set not_login=0 where not_login =1;
...
select * from user where user_id = 'INHE-0091' or user_id = 'INHE-0051'
select * from user where user_name like '%林鑫昊%'; 
select * from department where dept_id = 106

desc inhe_customer_contact
select * from inhe_customer_data_history;
-- CREATE TABLE IF NOT EXISTS inhe_customer_data_like (LIKE inhe_customer_data); 
select * from inhe_customer_data  order by id desc
select * from inhe_customer_data_history where form_id = 'khxx202101300032'
select * from inhe_customer_contact  order by id desc
select * from inhe_customer_company   order by id desc
-- create table inhe_customer_company_history as select * from inhe_customer_company
...
select * from inhe_project_main_history  order by id desc
...
desc inhe_customer_info
-- select * from inhe_project_code_ip;
select * from inhe_project_main  where form_id = 'lxdj2021013001'  if_project='01' and project_star = '05' and star_icon = '' and if_project='02'
update inhe_project_main a left join inhe_project_code_ip b on a.title = b.title set a.project_star = b.project_star,a.star_icon=b.star_icon,if_project=if_agree
select * from inhe_customer_info where form_id = 'lxdj2020122901'
select * from inhe_project_info  where form_id = 'lxdj2020122901'
select * from inhe_project_detail  where form_id = 'lxdj2020122901'
select * from inhe_bidding_strategy  where form_id = 'lxdj2020122901'
select * from inhe_project_manager 
select * from inhe_project_code_ip
select * from inhe_temp_code where temp_code like 'B%';
select * from mail -- PM_ID
select * from star_pm -- PM_ID
select * from inhe_oa_paras
-- sales_contract_review
select * from crs_tabledata696
select * from inhe_contract_review  where form_id = 'INHESC2021012201';  -- company
...
select * from inhe_contract_settle where form_id = 'INHESC2021012901';
select * from inhe_contract_settle_temp where form_id = 'INHESC2021012201';
select * from inhe_contract_settle_model;
create table  inhe_contract_settle_history as select * from inhe_contract_settle 
-- 主表、电表类明细、电表明细表2、电表明细表3、非电表类产品
-- order 
desc inhe_production_order
select * from inhe_production_order where form_id = 'scbg2021013002'
select * from inhe_production_order where form_id = 'sctz2021011801'
select * from inhe_production_detail_one where form_id = 'sctz2021011801'
select * from inhe_production_detail_two where form_id = 'sctz2021011801'
select * from inhe_production_detail_three where form_id = 'sctz2021011801'
select * from inhe_production_detail_non where form_id = 'sctz2021011801'
-- select * from inhe_production_detail2
...
-- select * from inhe_order_detail_temp where item_no  is null  where data_no = 2;
-- insert into inhe_order_detail_tmp  where item_no  is null
-- select * from inhe_order_detail  where form_id = 'sctz2020113005'
-- delete from inhe_order_detail where item_id = 'other_instruction'

update inhe_order_detail a left join inhe_detail_item b  on a.table_id = b.table_id and a.item_id = b.item_id
set a.item_no = b.sort_code
select * from inhe_detail_item where item_id = 'other_instruction'
-- 主表、电表类产品发货明细、非电表类产品明细、软件明细
desc inhe_shipping_detail
select * from inhe_shipping_notice where form_id = 'fhtz2021012105';
select * from inhe_shipping_detail where form_id = 'fhtz2021011801';
select * from inhe_oa_paras where type = 'currency'
-- 项目参与人	
...
desc inhe_project_participant_item
select * from inhe_number_participant_item order by id desc
-- update inhe_project_participant_item a left join user u on u.user_name = a.user_name set a.user_id = u.user_id 
-- 销售收款登记
-- crs_tabledata1949 - 跟单审核
select * from inhe_sales_receipt
desc inhe_sales_receipt
desc inhe_sales_receipt_detail
select * from inhe_sales_receipt where title like '%J.N.F.X.LIMITED%';
select * from inhe_sales_receipt_detail where order_number = 'INHESC2021012002' form_id like '%xssk2021020301%'
select * from inhe_sales_receipt_attach where form_id = 'xssk2021020301'
select * from inhe_attachment where relatedId = 'xssk2021020302'
select * from inhe_sales_receipt_attach_temp
select * from inhe_oa_paras where type like '%crm_process_type%'

-- 合同/协议登记表 174  东北非NEAC  INHE-0642  潘正豪
select * from inhe_contract_protocol where id = 280
...
-- customer_data  project_review  sales_contract_review  production_order  shipping_notice
-- 11	1	2	1	申请人		2	add							customer_data	客户信息登记	INHE
select * from inhe_flow_manage where type = 'customer_data' order by step;
select * from inhe_flow_manage where type = 'shipping_notice' order by step
select * from inhe_flow_opinion where flow_id = 'khxx202010121157' ;
select * from inhe_oa_paras where type_desc like '%管理部门%'
select * from user where user_id='WL-0048'
...
select * from rsyd where flow_id = 'RY20200526007'

select * from inhe_hr_flow_prcs where flow_id = 'RY20200526007'

select * from inhe_flow_manage

select * from yijian1  where flow_id = 'RY20200526007'
select * from canzhao
...
select * from sys_function 
select * from user_priv where func_id_str like '%1050%'
select * from inhe_system_admin

select * from inhe_oa_paras where type_desc like '%智能配电产品%'  
