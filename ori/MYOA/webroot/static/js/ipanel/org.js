...
function xmlDom2TreeNodes(dom, recursive, flag)
{
   ...
   var nodes = [];
   var node = node_name = node_id = node_title = is_org = user_id = dept_id = sex = url = uid_index = client = null;
...
function mouseenter_org0_node_callback()
{
   ...
   if(node.data.op_sms || typeof(node.data.client) != 'undefined' && node.data.client != '-1' && node.data.client != '2')
   {
      if(jQuery("#" + node.data.key + '_op').length <= 0)
      {
         ...
         if(node.data.op_sms)
         {
            html += '&nbsp;<img src="/static/images/msg.png" title="'+td_lang.inc.msg_135+'" onclick="parent.send_sms(\''+node.data.uid+'\',\''+ node.data.title +'\')" style="cursor:pointer;" />';
