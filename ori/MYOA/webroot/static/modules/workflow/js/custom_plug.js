...
function selectAll(type)
{
    if(type=='left'){
        ...
        var objSelectLength=jQuery('#next_step_tab').find('.select').length;
        ...
        objSelectLength=jQuery('#alternative_tab').find('.select').length;
...
//--------全选与取消全选结束-------------------------------------
//自定义插件的处理----
function selectPlug(tabId,tabIdOther,btnId)
{
    var s_tabIdOther = tabIdOther;
    var select=jQuery('.select').length;
    if(select==0){
        return false;
    }else{
        var parentTrLength=jQuery('#'+tabId).find('tr').length;
        var parentTr=jQuery('#'+tabId).find('tr');
        if(parentTr.hasClass('select')){
            for(var i=0;i<parentTrLength;i++){
                if(parentTr.eq(i).hasClass('select')==true){
                    ...
                    var cloneTr=parentTr.eq(i).removeClass('select');
                    if(cloneTr.length>0){
...
//---------插件选中的判断 以及全选以及取消的判断--------------------------
function selectTr(obj,leftTabId,rightTabtId,btnLeftId,btnRightId)
{
    var is_fire=getOs();
    ...
    var rightSelectLength=jQuery('#'+leftTabId).find('.select').length;
    var leftSelectLength=jQuery('#'+rightTabtId).find('.select').length;
    var rightTrLength=jQuery('#'+leftTabId).find('tr').length;
    var leftTrLength=jQuery('#'+rightTabtId).find('tr').length;
    if(jQuery(obj).hasClass('select')){
        if(rightSelectLength>1||leftSelectLength>1){//如果是当前是多选情况去掉出当前对象未的所有被选中的同辈元素
            silibings.removeClass('select');
        }else{
            jQuery(obj).removeClass('select');
        }
