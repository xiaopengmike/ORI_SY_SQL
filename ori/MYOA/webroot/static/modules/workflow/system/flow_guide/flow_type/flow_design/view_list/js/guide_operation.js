...
function auto_set()
{
	...
	if(auto_type=="3")
	{
		jQuery("#inteligent_auto_user_set").css('display', 'block');
	}else{
		jQuery("#inteligent_auto_user_set").css('display', 'none');
	}
	...
	if(auto_type==7)
	{
		jQuery("#inteligent_item_id_hide").css('display', 'block');
	}else{
		jQuery("#inteligent_item_id_hide").css('display', 'none');
	}
}
//条件设置判断显隐
...
function prcs_checkForm()
{
	...
	var operator_item_id = jQuery("#operator_item_id").find("span").attr("value_id");
	jQuery("input[name='ITEM_ID']").val(operator_item_id);
