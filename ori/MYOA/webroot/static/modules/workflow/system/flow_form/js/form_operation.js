...
{	
	...
	window.open("cool_form/?FORM_ID="+form_id,"cool_form","menubar=0,toolbar=0,scrollbars=no,status=0,resizable=1");
			...
			{			
				for(var key in jsonData[one])
				{
					if(model_icon == 2) //列表视图
					{
						if(i==0)
						{
							jQuery('#form_list1').find('table').append('<tr><td><a href="javascript:;"><span class="icon16-pencil-folder" title="'+td_lang.general.workflow.msg_172+'" sort_parent="'+jsonData[one][key].sort_parent+'" sort_id="'+jsonData[one][key].sort_id+'" onclick="form_sort_parent('+jsonData[one][key].sort_id+',\''+jsonData[one][key].sort_name+'\')">'+jsonData[one][key].sort_name+'</span></a></td><td><a class="form_ahref" href="javascript:form_sort_parent('+jsonData[one][key].sort_id+',\''+jsonData[one][key].sort_name+'\')">'+td_lang.general.workflow.msg_63+'</a></td></tr>');
							K++;
						}else if(i==1){
